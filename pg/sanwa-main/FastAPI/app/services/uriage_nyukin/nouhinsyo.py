...
class NouhinsyoService(BaseService):
    """納品書サービス"""

    def display(self, request, session) -> StreamingResponse:

        # 納品書の印刷処理
        logger.info("【START】納品書処理")
        ...
        with get_excel_buffer() as buffer:
            ...
            # 担当者cls取得
            cTanto = ClsTanto()
            tanto_result = cTanto.GetbyID(session,storedresults['results'][0][0]["担当者CD"])
            mail_address = tanto_result[0]["メールアドレス"] if len(tanto_result) == 1 else ""
            ...
            for row_hedder_data in storedresults['results'][0]:
                ...
                zeiritu = str(NouhinsyoService.getZeiritu(session, request))
                ws['A10'] = '消費税等(' + zeiritu + '%)'
                ...
                if str(row_hedder_data['納期表示']) == '0':
                    if str(row_hedder_data['納期S']) != 'None':
                        if str(row_hedder_data['納期E']) != 'None':
                            ws['D15'].value = "{}～{}".format(
                                datetime.strftime(row_hedder_data["納期S"],'%Y{}%m{}%d{}'.format('年','月','日')),
                                datetime.strftime(row_hedder_data["納期E"],'%Y{}%m{}%d{}'.format('年','月','日')),
                            )
                        else:
                            ws['D15'] = datetime.strftime(row_hedder_data["納期S"],'%Y{}%m{}%d{}'.format('年','月','日'))
                    else:
                        if str(row_hedder_data['納期E']) != 'None':
                            ws['D15'] = datetime.strftime(row_hedder_data["納期E"],'%Y{}%m{}%d{}'.format('年','月','日'))
                ...
                # 三和情報追加
                company_info = NouhinsyoService.getKaisya(session, request)
                ...
                if row_hedder_data['担当者名'] is not None:
                    ws['L16'] = '問い合わせ先：' + row_hedder_data['担当者名']
                else:
                    ws['L16'] = ''
                ws['L17'] = 'E-mail:{}'.format(mail_address)

                mitomori_no = row_hedder_data['見積番号']
    ...
    def display_buppin(self, request, session) -> StreamingResponse:
        ...
        with get_excel_buffer() as buffer:
            ...
            for row_siwake_data in storedresults['results'][1]:
                ...
                if page_break_flg:
                    ...
                    range_copy_cell(ws,ws,1,30,13,59,0,row_num-32,False)
                    ws.unmerge_cells('B{t1}:D{t1}'.format(t1=str(row_num-1)))
                    ws.unmerge_cells('L{t1}:M{t1}'.format(t1=str(row_num-1)))
                    range_copy_cell(ws,ws,1,row_num-31,13,row_num-31,0,30,True)
                    relative_reference_copy(ws,ws,9,row_num-30,0,30)
                    relative_reference_copy(ws,ws,12,row_num-30,0,30)
                    page_row_num = 1
                ...
                for row_data in storedresults['results'][2]:
                    if row_siwake_data['仕分番号'] == row_data['仕分番号']:
                        ...
                        #2行目以降なら行コピー
                        if page_row_num != 1:
                            range_copy_cell(ws,ws,1,row_num-1,10,row_num-1,0,1,False)
                            relative_reference_copy(ws,ws,9,row_num-1,0,1)
                            relative_reference_copy(ws,ws,12,row_num-1,0,1)
                        ws.cell(row=row_num, column=1, value=row_data['行番号'])._style = copy(ws.cell(row=start_row, column=1)._style)
                        if row_data['見積区分'] == 'C' or row_data['見積区分'] == 'A' or row_data['見積区分'] == 'S':
            ...
            if request.params['type'] == 'pdf':
                ...
                if page_num > 1:
                    ...
                    for i in range(31,59):
                        ws.unmerge_cells('B{t}:D{t}'.format(t=i))
                        ws.unmerge_cells('L{t}:M{t}'.format(t=i))
                    # 余計な行を削除
                    ws.delete_rows(31, 60)
                    e_cell = "M29"
