...
class MitsumorisyoService(BaseService):
    def display(self, request, session):
        ...
        # ストアプロシージャだし分け
        stored_name = ""
        format_category = request.params["書式区分"]
        if format_category in ["0", "1"]:
            stored_name = "usp_MT0300見積書出力"
        elif format_category == "2":
            stored_name = "usp_MT0301見積書出力_客在"
        elif format_category == "3":
            stored_name = "usp_MT0302見積書出力_部材計"
        elif format_category == "4":
    ...
    def populate_data_to_sheet(self,session, ws,copy_ws, array, request, min_mode=False):
        ...
        if min_mode:
            ...
            # 第二配列のデータ挿入
            current_row = self.insert_second_array_data(ws, copy_ws,array[1], 21, 1,request)

            # 第三配列のデータ挿入
            self.insert_third_array_group_data(ws,copy_ws, array, current_row, 1,request)
            # self.insert_third_array_group_data(ws,copy_ws, array, 41, 1,request)
    ...
    def insert_second_array_data(self, ws, copy_ws,second_array, start_row, start_col,request):
        """
        ...
        for item in second_array:
            if current_row == 38:
                ...
                ws.cell(row=current_row,column=17,value="次ページへ").alignment = Alignment(horizontal="center")
                range_copy_cell_by_address(copy_ws,ws,'A1','O35','A39',True)
                current_row = 42
            # 各データを列に挿入
            ...
            # スタイル
            ws.cell(row=current_row, column=start_col + 2)._style = copy(ws.cell(row=21,column=3)._style)
        ...
        total_cell.alignment = Alignment(horizontal='center', vertical='center')  # 水平方向、垂直方向両方センタリング
    ...
    def insert_row_data(self, ws,copy_ws, row, col, item,request):
        ...
        # self.write_cell(ws, row, col + 10, self.get_value_or_empty(item, "数量"))
        ...
        if request.params["書式区分"] != "1":
            ...
            # self.write_cell(ws, row, col + 13, self.get_value_or_empty(item, "売上単価"))
