...
class SeikyumeisaiitiranhyoService(BaseService):
    """請求明細一覧表サービス"""

    def display(self, request, session) -> StreamingResponse:

        # 支払予定表の印刷処理
        logger.info("【START】請求明細一覧表処理")

        storedname="usp_SE1300請求明細一覧表"
        ...
        with get_excel_buffer() as buffer:
            wb = create_excel_object('Template請求明細一覧表.xlsx')
            ...
            for ws in wb.worksheets:
                ...
                # 作成日の追加
                ws['H3'] = datetime.today().strftime('%Y{0}%m{1}%d{2}').format(*'年月日')
