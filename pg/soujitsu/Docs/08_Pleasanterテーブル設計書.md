...
**想定レコード数**: 約100件（アクティブテナント）  
**更新頻度**: 低（月1回）

**注記**：
- 実テナント数は450件存在するが、初期ではアクティブなテナント約100件を登録
- 必要に応じて段階的にテナントを追加予定
...
**想定レコード数**: 約700件/月（年間8,400件）  
**更新頻度**: 高（日次）

**注記**：
- 実テナント450件が複数の電力メーターを持つため、検針データ件数は約700件/月となる
...
**注記**：
- 過去3年分実績には検針データ（8,400件×3年）と関連トランザクションを含む
...
**注記**：
- 検針データの増加により、当初想定より年間3,000件増加

### 5年後の想定規模
- 総レコード数: 約125,000件
- データベースサイズ: 約600MB（添付ファイル除く）
- 添付ファイル込み: 約6GB
