...
Friend Class SnwMT01F00
	...
	'別画面より呼び出しの場合
	Dim pParentForm As System.Windows.Forms.Form '呼び出し親フォーム
	...
	Private Sub SnwMT01F00_FormClosing(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
		...
		If Loaded Then
			If pEndMsg Then
				If MsgBoxResult.Yes = NoYes("現在の処理を終了します。") Then
					...
					'ロック情報を解除する。
					'UPGRADE_WARNING: オブジェクト SpcToNull() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
					Call UnLockData("見積番号", SpcToNull([rf_見積番号], 0))
					pParentForm.Show()
	...
	Private Sub SnwMT01F00_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
		If Me.WindowState <> System.Windows.Forms.FormWindowState.Minimized Then
			...
			If VB6.PixelsToTwipsY(Me.Height) < MeHeight Then
				Me.Height = VB6.TwipsToPixelsY(MeHeight)
			End If
		End If
	End Sub
	
	Private Sub SnwMT01F00_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		Loaded = False
		pEndMsg = True
		...
		MeHeight = VB6.PixelsToTwipsY(Me.Height)
	...
	Private Sub SnwMT01F00_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
...
Form_KeyDown_Err: 
	...
	Private Sub SnwMT01F00_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
	...
	Private Function Item_Check(ByRef ItemNo As Short) As Short
		...
		'UPGRADE_WARNING: Null/IsNull() の使用が見つかりました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="2EED02CB-5C0E-4DC1-AE94-4FAA3A30F51A"' をクリックしてください。
		'UPGRADE_WARNING: オブジェクト HBUKKENNO の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		If ItemNo > [tx_物件番号].TabIndex And ([tx_物件番号].Text <> HBUKKENNO Or IsDbNull(HBUKKENNO)) Then
			...
			If IsCheckText([tx_物件番号]) = True Then
				...
				With cBukken
					...
					Else
						...
						If NullToZero((idc(0).Text), "") = VB6.Format(Today, "yyyy/mm/dd") Then
							'初期値ならば変更
		...
		If ItemNo > tx_得意先CD.TabIndex And ([tx_得意先CD].Text <> HTOKUCD Or IsDbNull(HTOKUCD)) Then
			...
			If ISInt(([tx_得意先CD].Text)) Then
				...
				Chk_ID = VB6.Format([tx_得意先CD].Text, New String("0", [tx_得意先CD].Maxlength))
			Else
		...
		If ItemNo > [tx_集計CD].TabIndex And ([tx_集計CD].Text <> HSYUKEICD Or IsDbNull(HSYUKEICD)) Then
			...
			If ISInt(([tx_集計CD].Text)) Then
				...
				Chk_ID = VB6.Format([tx_集計CD].Text, New String("0", [tx_集計CD].Maxlength))
			Else
				Chk_ID = [tx_集計CD].Text
			End If
		...
		If ItemNo > [tx_納得意先CD].TabIndex And ([tx_納得意先CD].Text <> HNTOKUCD Or IsDbNull(HNTOKUCD)) Then
			...
			If ISInt(([tx_納得意先CD].Text)) Then
				...
				Chk_ID = VB6.Format([tx_納得意先CD].Text, New String("0", [tx_納得意先CD].Maxlength))
			Else
		...
		If ItemNo > [tx_納入先CD].TabIndex And ([tx_納入先CD].Text <> HNONYUCD Or IsDbNull(HNONYUCD)) Then
			...
			If ISInt(([tx_納入先CD].Text)) Then
				...
				Chk_ID = VB6.Format([tx_納入先CD].Text, New String("0", [tx_納入先CD].Maxlength))
			Else
		...
		If [tx_得意先CD].Text = "9999" Or [tx_得意先CD].Text = "3055" Then '2015/01/30 ADD
			...
			If ItemNo > [tx_販売先得意先CD].TabIndex And ([tx_販売先得意先CD].Text <> H_HANTOKUCD Or IsDbNull(H_HANTOKUCD)) Then
				...
				If ISInt(([tx_販売先得意先CD].Text)) Then
					...
					Chk_ID = VB6.Format([tx_販売先得意先CD].Text, New String("0", [tx_販売先得意先CD].Maxlength))
				Else
			...
			If ItemNo > [tx_販売先納得意先CD].TabIndex And ([tx_販売先納得意先CD].Text <> H_HANNTOKUCD Or IsDbNull(H_HANNTOKUCD)) Then
				...
				If ISInt(([tx_販売先納得意先CD].Text)) Then
					...
					Chk_ID = VB6.Format([tx_販売先納得意先CD].Text, New String("0", [tx_販売先納得意先CD].Maxlength))
				Else
			...
			If ItemNo > [tx_販売先納入先CD].TabIndex And ([tx_販売先納入先CD].Text <> H_HANNONYUCD Or IsDbNull(H_HANNONYUCD)) Then
				...
				If ISInt(([tx_販売先納入先CD].Text)) Then
					...
					Chk_ID = VB6.Format([tx_販売先納入先CD].Text, New String("0", [tx_販売先納入先CD].Maxlength))
				Else
...
Get売上_出精値引_Err: 
	...
	Private Function Download(ByRef No As Integer) As Integer
		Dim rs As ADODB.Recordset
		Dim sql As String
		Dim X As Short
		Dim buf As String
		Dim KANNOOF As Short
		...
		With rs
			...
			Else
				...
				idc(0).Text = .Fields("見積日付")
				tx_見積件名.Text = .Fields("見積件名").Value
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_得意先CD].Text = NullToZero(.Fields("得意先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_得意先名1.Text = NullToZero(.Fields("得意先名1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_得意先名2.Text = NullToZero(.Fields("得意先名2"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_得TEL.Text = NullToZero(.Fields("得TEL"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_得FAX.Text = NullToZero(.Fields("得FAX"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_得担当者.Text = NullToZero(.Fields("得意先担当者"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納得意先CD.Text = NullToZero(.Fields("納入得意先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納入先CD.Text = NullToZero(.Fields("納入先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納入先名1.Text = NullToZero(.Fields("納入先名1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納入先名2.Text = NullToZero(.Fields("納入先名2"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_郵便番号.Text = NullToZero(.Fields("郵便番号"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納住所1.Text = NullToZero(.Fields("住所1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納住所2.Text = NullToZero(.Fields("住所2"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納TEL.Text = NullToZero(.Fields("納TEL"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納FAX.Text = NullToZero(.Fields("納FAX"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納担当者.Text = NullToZero(.Fields("納入先担当者"), "")
				idc(1).Text = .Fields("納期S")
				idc(2).Text = .Fields("納期E")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_備考.Text = NullToZero(.Fields("備考"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_物件金額.Text = NullToZero(.Fields("物件規模金額"), "")
				idc(3).Text = .Fields("オープン日")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_物件種別.Text = NullToZero(.Fields("物件種別"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_伝票種類.Text = NullToZero(.Fields("伝票種類"), "") '2015/02/04 ADD
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_SM内容区分.Text = NullToZero(.Fields("SM内容区分"), "") '2015/11/19 ADD
				
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_現場名.Text = NullToZero(.Fields("現場名"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_支払条件.Text = NullToZero(.Fields("支払条件"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_支払条件他.Text = NullToZero(.Fields("支払条件その他"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納期表示.Text = NullToZero(.Fields("納期表示"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_納期表示他.Text = NullToZero(.Fields("納期その他"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_出力日.Text = NullToZero(.Fields("見積日出力"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_有効期限.Text = NullToZero(.Fields("有効期限"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_受注区分.Text = NullToZero(.Fields("受注区分"), "")
				idc(4).Text = .Fields("受注日付")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_大小口区分.Text = NullToZero(.Fields("大小口区分"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_出精値引.Text = NullToZero(.Fields("出精値引"), "")
				
				'2016/04/07 ADD↓
				''            rf_税集計区分.Caption = ![税集計区分]
				tx_税集計区分.Text = .Fields("税集計区分").Value
				[rf_税集計区分名].Text = cTokuisaki.Get税集計区分名(.Fields("税集計区分").Value)
				'2016/04/07 ADD↑
				...
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				rf_得意先別見積番号.Text = NullToZero(.Fields("得意先別見積番号"), "") '2005/07/04 ADD
				...
				'2013/03/08 ADD↓
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_部署CD].Text = NullToZero(.Fields("部署CD"))
				With cBusyo
					.Initialize()
					.[部署CD] = tx_部署CD.Text
					.GetbyID()
				...
				'2022/08/08 ADD↓
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_工事担当CD.Text = NullToZero(.Fields("工事担当CD"), "")
				With cKoujiTanto
					.Initialize()
					.[工事担当CD] = tx_工事担当CD.Text
					.GetbyID()
				...
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_販売先得意先CD.Text = NullToZero(.Fields("販売先得意先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				rf_販売先得意先名1.Text = NullToZero(.Fields("販売先得意先名1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				rf_販売先得意先名2.Text = NullToZero(.Fields("販売先得意先名2"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_販売先納得意先CD.Text = NullToZero(.Fields("販売先納得意先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_販売先納入先CD.Text = NullToZero(.Fields("販売先納入先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				rf_販売先納入先名1.Text = NullToZero(.Fields("販売先納入先名1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				rf_販売先納入先名2.Text = NullToZero(.Fields("販売先納入先名2"), "")
				
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_部署CD = NullToZero(.Fields("部署CD"))
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先得意先CD = NullToZero(.Fields("販売先得意先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先得意先名1 = NullToZero(.Fields("販売先得意先名1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先得意先名2 = NullToZero(.Fields("販売先得意先名2"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先納得意先CD = NullToZero(.Fields("販売先納得意先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先納入先CD = NullToZero(.Fields("販売先納入先CD"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先納入先名1 = NullToZero(.Fields("販売先納入先名1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_販売先納入先名2 = NullToZero(.Fields("販売先納入先名2"), "")
				
				'UPGRADE_WARNING: オブジェクト HBUSYOCD の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HBUSYOCD = IIf(CDbl(tx_部署CD.Text) = 0, "", [tx_部署CD].Text)
				'UPGRADE_WARNING: オブジェクト SpcToNull() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト H_HANTOKUCD の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				...
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_工事担当CD = NullToZero(.Fields("工事担当CD")) '2022/08/08 ADD
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト HKOUJITANTOCD の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HKOUJITANTOCD = NullToZero(.Fields("工事担当CD")) '2022/08/08 ADD
				
				'2013/03/08 ADD↑
				
				'2013/07/19 ADD↓
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				ck_社内伝票扱い.CheckState = NullToZero(.Fields("社内伝票扱い"), 0)
				'2013/07/19 ADD↑
				...
				'2015/06/12 ADD↓
				[tx_ウエルシア物件内容CD].Text = VB6.Format(.Fields("ウエルシア物件内容CD").Value, "#")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_ウエルシア物件内容名].Text = NullToZero(.Fields("ウエルシア物件内容名"), "")
				
				tx_ウエルシア売場面積.Text = VB6.Format(.Fields("ウエルシア売場面積").Value, "#")
				
				[tx_ウエルシアリース区分].Text = VB6.Format(.Fields("ウエルシアリース区分").Value, "#")
				
				[tx_ウエルシア物件区分].Text = VB6.Format(.Fields("ウエルシア物件区分CD").Value, "#")
				With cWelBukkenKubun
					.Initialize()
					.[ウエルシア物件区分CD] = tx_ウエルシア物件区分.Text
					.GetbyID()
				...
				'2015/07/10 ADD↓
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_物件番号].Text = NullToZero(.Fields("物件番号"))
				'UPGRADE_WARNING: オブジェクト HBUKKENNO の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HBUKKENNO = tx_物件番号.Text
				
				With cBukken
					.Initialize()
					.[物件番号] = CInt(tx_物件番号.Text)
					.GetbyID()
					rf_物件略称.Text = .物件略称
				End With
				...
				'2024/01/16 ADD↓
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_統合見積番号.Text = NullToZero(.Fields("統合見積番号"))
				'UPGRADE_WARNING: オブジェクト H統合見積NO の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				H統合見積NO = tx_統合見積番号.Text
				...
				'2015/10/16 ADD↓
				idc(5).Text = .Fields("受付日付")
				idc(6).Text = .Fields("完工日付")
				tx_発注担当者名.Text = .Fields("発注担当者名").Value
				[tx_作業内容].Text = .Fields("作業内容").Value
				[tx_YKサプライ区分].Text = VB6.Format(.Fields("YKサプライ区分").Value, "#")
				[tx_YK物件区分].Text = VB6.Format(.Fields("YK物件区分").Value, "#")
				[tx_YK請求区分].Text = VB6.Format(.Fields("YK請求区分").Value, "#")
				'2015/10/16 ADD↑
				
				'2015/11/03 ADD↓
				[tx_化粧品メーカー区分].Text = VB6.Format(.Fields("化粧品メーカー区分").Value, "#")
				'2015/11/03 ADD↑
				
				[tx_クレーム区分].Text = VB6.Format(.Fields("クレーム区分").Value, "#") '2016/06/22 ADD
				
				
				'2020/04/11 ADD↓
				[tx_発注書発行日付].Text = VB6.Format(.Fields("発注書発行日付").Value, "yyyy年mm月dd日")
				
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_見積確定区分].Text = NullToZero(.Fields("見積確定区分"), "")
				
				idc(7).Text = .Fields("完了日付")
				'UPGRADE_WARNING: オブジェクト idc().Text の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				...
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_完了者名].Text = NullToZero(.Fields("完了者名"), "")
				
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				HD_入力USERID = NullToZero(.Fields("入力USERID"), "")
				
				idc(8).Text = .Fields("請求予定日")
				'UPGRADE_WARNING: オブジェクト idc().Text の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				...
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_経過備考1].Text = NullToZero(.Fields("経過備考1"), "")
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_経過備考2.Text = NullToZero(.Fields("経過備考2"), "")
				
				'2020/04/11 ADD↑
				
				'2021/02/25 ADD↓
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				tx_集計CD.Text = NullToZero(.Fields("集計CD"), "")
				With cSyuukei
					.Initialize()
					.[得意先CD] = tx_集計CD.Text
					.GetbyID()
				...
				'2022/10/10 ADD↓
				[tx_B請求管轄区分].Text = VB6.Format(.Fields("B請求管轄区分").Value, "#")
				[tx_BtoB番号].Text = VB6.Format(.Fields("BtoB番号").Value, "#")
				'2022/10/10 ADD↑
				...
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				[tx_業種区分].Text = NullToZero(.Fields("業種区分"), "") '2023/04/18 ADD
		...
		If isExist請求売上(No) = True Then
			'請求済み
			'変更不可にする
			[tx_完了日Y].ReadOnly = True
			[tx_完了日M].ReadOnly = True
			[tx_完了日D].ReadOnly = True
			[tx_請求予定Y].ReadOnly = True
			[tx_請求予定M].ReadOnly = True
			[tx_請求予定D].ReadOnly = True
			[tx_経過備考1].ReadOnly = True
			[tx_経過備考2].ReadOnly = True
		Else
			'未請求
			[tx_完了日Y].ReadOnly = False
			[tx_完了日M].ReadOnly = False
			[tx_完了日D].ReadOnly = False
			[tx_請求予定Y].ReadOnly = False
			[tx_請求予定M].ReadOnly = False
			[tx_請求予定D].ReadOnly = False
			[tx_経過備考1].ReadOnly = False
			[tx_経過備考2].ReadOnly = False
		End If
...
Download_Err: 
	...
	'2015/07/10 ADD↓
	Private Sub tx_物件番号_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_物件番号.Enter
		...
		[sb_Msg].Items.Item(1).Text = "物件番号を入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_物件番号_SpcKeyPress(ByRef KeyAscii As Short, ByRef Cancel As Boolean)
		If KeyAscii = Asc(" ") And ([tx_物件番号].SelectionStart = 0 And [tx_物件番号].SelectionLength = Len([tx_物件番号].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
	...
	Private Sub tx_物件番号_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_物件番号_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_物件番号.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
	...
	'2024/01/16 ADD↓
	Private Sub tx_統合見積番号_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_統合見積番号.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "統合見積番号を入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_統合見積番号_SpcKeyPress(ByRef KeyAscii As Short, ByRef Cancel As Boolean)
		If KeyAscii = Asc(" ") And ([tx_統合見積番号].SelectionStart = 0 And [tx_統合見積番号].SelectionLength = Len([tx_統合見積番号].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
	...
	Private Sub tx_統合見積番号_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_統合見積番号_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_統合見積番号.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_統合見積番号.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_担当者CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_担当者CD.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "担当者CDを入力して下さい。　選択画面：Space"
	End Sub
	...
	'2013/03/07 ADD↓
	Private Sub tx_部署CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_部署CD.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "部署CDを入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_部署CD_SpcKeyPress(ByRef KeyAscii As Short, ByRef Cancel As Boolean)
		If KeyAscii = Asc(" ") And ([tx_部署CD].SelectionStart = 0 And [tx_部署CD].SelectionLength = Len([tx_部署CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
	...
	Private Sub tx_部署CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_部署CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_部署CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_部署CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_見積日付Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_見積日付Y.Enter
		...
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "見積日付を入力して下さい。"
		PreviousControl = Me.ActiveControl
		'ボタン名設定
		'UPGRADE_ISSUE: Control Name は、汎用名前空間 ActiveControl 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
		Call SetUpFuncs(Me.ActiveControl.Name)
	...
	Private Sub tx_見積日付Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_見積日付Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_見積日付Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_見積日付Y.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_見積日付M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_見積日付M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_見積日付M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_見積日付M.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_見積日付D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_見積日付D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		''''    If KeyAscii <> 8 Then ' バックスペースは例外
		''''        If LenB(StrConv([tx_見積日付D].Text & Chr$(KeyAscii), vbFromUnicode)) - _
		'''''            LenB(StrConv([tx_見積日付D].SelText, vbFromUnicode)) = [tx_見積日付D].MaxLength Then
		''''            If KeyAscii <> 0 Then       '入力キーがエラーでないならば
		''''                    ReturnF = True
		''''                    [tx_見積日付D].SetFocus
		''''            End If
	...
	Private Sub tx_見積日付D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_見積日付D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_見積日付D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_見積日付D.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	'2022/08/08 ADD↓
	Private Sub tx_工事担当CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_工事担当CD.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "工事担当を入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_工事担当CD_SpcKeyPress(ByRef KeyAscii As Short, ByRef Cancel As Boolean)
		If KeyAscii = Asc(" ") And ([tx_工事担当CD].SelectionStart = 0 And [tx_工事担当CD].SelectionLength = Len([tx_工事担当CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
	...
	Private Sub tx_工事担当CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_工事担当CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_工事担当CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_工事担当CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	'''''    'ボタン名設定
	'''''    Call SetUpFuncs(Me.ActiveControl.Name)
	'''''    [sb_Msg].Panels(1).Text = "積算担当を入力して下さい。　選択画面：Space"
	'''''End Sub
	'''''
	'''''Private Sub tx_営業推進部CD_SpcKeyPress(KeyAscii As Integer, Cancel As Boolean)
	...
	'''''End Sub
	'''''
	'''''Private Sub tx_営業推進部CD_RtnKeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
	'''''    ReturnF = True
	'''''End Sub
	'''''
	...
	Private Sub tx_見積件名_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_見積件名.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "見積件名を入力して下さい。"
	End Sub
	...
	Private Sub tx_得意先CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得意先CD.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "得意先CDを入力して下さい。　選択画面：Space"
	End Sub
	...
	Private Sub tx_得意先名1_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得意先名1.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "得意先名1を入力して下さい。"
	End Sub
	
	Private Sub tx_得意先名1_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_得意先名1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得意先名1.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_得意先名1.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_得意先名2_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得意先名2.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "得意先名2を入力して下さい。"
	End Sub
	
	Private Sub tx_得意先名2_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_得意先名2_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得意先名2.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_得TEL_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得TEL.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ＴＥＬを入力して下さい。"
	End Sub
	
	Private Sub tx_得TEL_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_得TEL_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得TEL.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_得FAX_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得FAX.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ＦＡＸを入力して下さい。"
	End Sub
	
	Private Sub tx_得FAX_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_得FAX_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得FAX.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_得担当者_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得担当者.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "得意先担当者名を入力して下さい。"
	End Sub
	
	Private Sub tx_得担当者_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_得担当者_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_得担当者.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2016/04/07 ADD↓
	Private Sub tx_税集計区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_税集計区分.Enter
		If Item_Check(([tx_税集計区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "税集計区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_税集計区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_税集計区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_税集計区分].Text, vbFromUnicode)) = [tx_税集計区分].Maxlength - 1 Then
					Call SelText([tx_税集計区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_税集計区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_税集計区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_税集計区分].SelectedText, vbFromUnicode)) = [tx_税集計区分].Maxlength - 1 Then
					ReturnF = True
					[tx_税集計区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_税集計区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_税集計区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_税集計区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2021/02/25 ADD↓
	Private Sub tx_集計CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_集計CD.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "集計CDを入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_集計CD_SpcKeyPress(ByRef KeyAscii As Short, ByRef Cancel As Boolean)
		If KeyAscii = Asc(" ") And ([tx_集計CD].SelectionStart = 0 And [tx_集計CD].SelectionLength = Len([tx_集計CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
	...
	Private Sub tx_集計CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_集計CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_集計CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_集計CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_納得意先CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納得意先CD.Enter
		If SelectF = False Then
	...
	Private Sub tx_納得意先CD_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_納得意先CD.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii = Asc(" ") And ([tx_納得意先CD].SelectionStart = 0 And [tx_納得意先CD].SelectionLength = Len([tx_納得意先CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
			If cTokuisaki.ShowDialog = True Then
	...
	Private Sub tx_納得意先CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納得意先CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納得意先CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_納得意先CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_納入先CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納入先CD.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "納入先CDを入力して下さい。　選択画面：Space"
	End Sub
	...
	Private Sub tx_納入先名1_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納入先名1.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "納入先名１を入力して下さい。"
	End Sub
	
	Private Sub tx_納入先名1_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納入先名1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納入先名1.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_納入先名1.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_納入先名2_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納入先名2.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "納入先名２を入力して下さい。"
	End Sub
	
	Private Sub tx_納入先名2_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納入先名2_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納入先名2.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_郵便番号_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_郵便番号.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "郵便番号を入力して下さい。"
	End Sub
	
	Private Sub tx_郵便番号_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_郵便番号_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_郵便番号.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納住所1_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納住所1.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "住所1を入力して下さい。"
	End Sub
	
	Private Sub tx_納住所1_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納住所1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納住所1.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納住所2_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納住所2.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "住所2を入力して下さい。"
	End Sub
	
	Private Sub tx_納住所2_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納住所2_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納住所2.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納TEL_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納TEL.Enter
	...
	Private Sub tx_納TEL_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納TEL_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納TEL.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納FAX_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納FAX.Enter
	...
	Private Sub tx_納FAX_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納FAX_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納FAX.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納担当者_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納担当者.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "納入先担当者名を入力して下さい。"
	End Sub
	
	Private Sub tx_納担当者_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納担当者_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納担当者.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub ck_社内伝票扱い_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles ck_社内伝票扱い.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "社内伝扱いの場合はチェックを付けて下さい。"
	End Sub
	
	Private Sub ck_社内伝票扱い_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles ck_社内伝票扱い.KeyDown
		Dim KeyCode As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
	...
	Private Sub ck_社内伝票扱い_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles ck_社内伝票扱い.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		ReturnF = False
	End Sub
	...
	'2013/03/07 ADD↓
	Private Sub tx_販売先得意先CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_販売先得意先CD.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "販売先得意先CDを入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_販売先得意先CD_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_販売先得意先CD.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii = Asc(" ") And ([tx_販売先得意先CD].SelectionStart = 0 And [tx_販売先得意先CD].SelectionLength = Len([tx_販売先得意先CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
			'---参照画面表示
	...
	Private Sub tx_販売先得意先CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_販売先得意先CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_販売先得意先CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_販売先得意先CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		...
		ReturnF = False
	End Sub
	Private Sub tx_販売先納得意先CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_販売先納得意先CD.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "販売先納入得意先CDを入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_販売先納得意先CD_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_販売先納得意先CD.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii = Asc(" ") And ([tx_販売先納得意先CD].SelectionStart = 0 And [tx_販売先納得意先CD].SelectionLength = Len([tx_販売先納得意先CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
			If cTokuisaki.ShowDialog = True Then
	...
	Private Sub tx_販売先納得意先CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_販売先納得意先CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_販売先納得意先CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_販売先納得意先CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_販売先納入先CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_販売先納入先CD.Enter
		If SelectF = False Then
	...
	Private Sub tx_販売先納入先CD_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_販売先納入先CD.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii = Asc(" ") And ([tx_販売先納入先CD].SelectionStart = 0 And [tx_販売先納入先CD].SelectionLength = Len([tx_販売先納入先CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
	...
	Private Sub tx_販売先納入先CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_販売先納入先CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_販売先納入先CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_販売先納入先CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_s納期Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_s納期Y.Enter
		'入力チェック
		If Item_Check(([tx_s納期Y].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "開始納期を入力して下さい。"
		PreviousControl = Me.ActiveControl
		'ボタン名設定
		'UPGRADE_ISSUE: Control Name は、汎用名前空間 ActiveControl 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
		Call SetUpFuncs(Me.ActiveControl.Name)
	...
	Private Sub tx_s納期Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_s納期Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_s納期Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_s納期Y].SelectedText, vbFromUnicode)) = [tx_s納期Y].Maxlength Then
				ReturnF = True
				[tx_s納期Y].Focus()
			End If
		End If
	...
	Private Sub tx_s納期Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_s納期Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_s納期Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_s納期Y.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_s納期M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_s納期M.Enter
		'入力チェック
	...
	Private Sub tx_s納期M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_s納期M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_s納期M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_s納期M].SelectedText, vbFromUnicode)) = [tx_s納期M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_s納期M].Focus()
				End If
	...
	Private Sub tx_s納期M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_s納期M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_s納期M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_s納期M.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_s納期D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_s納期D.Enter
		'入力チェック
	...
	Private Sub tx_s納期D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_s納期D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		''''    If KeyAscii <> 8 Then ' バックスペースは例外
		''''        If LenB(StrConv([tx_s納期D].Text & Chr$(KeyAscii), vbFromUnicode)) - _
		'''''            LenB(StrConv([tx_s納期D].SelText, vbFromUnicode)) = [tx_s納期D].MaxLength Then
		''''            If KeyAscii <> 0 Then       '入力キーがエラーでないならば
		''''                    ReturnF = True
		''''                    [tx_s納期D].SetFocus
		''''            End If
	...
	Private Sub tx_s納期D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_s納期D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_s納期D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_s納期D.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_e納期Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_e納期Y.Enter
		'入力チェック
		If Item_Check(([tx_e納期Y].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "終了納期を入力して下さい。"
		PreviousControl = Me.ActiveControl
		'ボタン名設定
		'UPGRADE_ISSUE: Control Name は、汎用名前空間 ActiveControl 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
		Call SetUpFuncs(Me.ActiveControl.Name)
	...
	Private Sub tx_e納期Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_e納期Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_e納期Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_e納期Y].SelectedText, vbFromUnicode)) = [tx_e納期Y].Maxlength Then
				ReturnF = True
				[tx_e納期Y].Focus()
			End If
		End If
	...
	Private Sub tx_e納期Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_e納期Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_e納期Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_e納期M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_e納期M.Enter
		'入力チェック
	...
	Private Sub tx_e納期M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_e納期M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_e納期M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_e納期M].SelectedText, vbFromUnicode)) = [tx_e納期M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_e納期M].Focus()
				End If
	...
	Private Sub tx_e納期M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_e納期M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_e納期M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_e納期D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_e納期D.Enter
		'入力チェック
	...
	Private Sub tx_e納期D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_e納期D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		''''    If KeyAscii <> 8 Then ' バックスペースは例外
		''''        If LenB(StrConv([tx_e納期D].Text & Chr$(KeyAscii), vbFromUnicode)) - _
		'''''            LenB(StrConv([tx_e納期D].SelText, vbFromUnicode)) = [tx_e納期D].MaxLength Then
		''''            If KeyAscii <> 0 Then       '入力キーがエラーでないならば
		''''                    ReturnF = True
		''''                    [tx_e納期D].SetFocus
		''''            End If
	...
	Private Sub tx_e納期D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_e納期D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_e納期D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	''''''End Sub
	''''''
	''''''Private Sub tx_納期hh_RtnKeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
	''''''    ReturnF = True
	''''''End Sub
	''''''
	...
	''''''End Sub
	''''''
	''''''Private Sub tx_納期nn_RtnKeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
	''''''    ReturnF = True
	''''''End Sub
	''''''
	...
	Private Sub tx_備考_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_備考.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "備考を入力して下さい。"
	End Sub
	
	Private Sub tx_備考_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_備考_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_備考.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_業種区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_業種区分.Enter
		...
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "業種区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
...
EventExitSub: 
	...
	Private Sub tx_物件金額_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_物件金額.Enter
		If Item_Check(([tx_物件金額].TabIndex)) = False Then
			Exit Sub
		End If
		
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "物件金額を入力して下さい。"
		PreviousControl = Me.ActiveControl
	End Sub
	
	Private Sub tx_物件金額_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_物件金額_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_物件金額.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_OPEN日Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_OPEN日Y.Enter
		'入力チェック
		If Item_Check(([tx_OPEN日Y].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "オープン日を入力して下さい。"
		PreviousControl = Me.ActiveControl
		'ボタン名設定
		'UPGRADE_ISSUE: Control Name は、汎用名前空間 ActiveControl 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
		Call SetUpFuncs(Me.ActiveControl.Name)
	...
	Private Sub tx_OPEN日Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_OPEN日Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_OPEN日Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_OPEN日Y].SelectedText, vbFromUnicode)) = [tx_OPEN日Y].Maxlength Then
				ReturnF = True
				[tx_OPEN日Y].Focus()
			End If
		End If
	...
	Private Sub tx_OPEN日Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_OPEN日Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_OPEN日Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_OPEN日M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_OPEN日M.Enter
		'入力チェック
	...
	Private Sub tx_OPEN日M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_OPEN日M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_OPEN日M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_OPEN日M].SelectedText, vbFromUnicode)) = [tx_OPEN日M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_OPEN日M].Focus()
				End If
	...
	Private Sub tx_OPEN日M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_OPEN日M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_OPEN日M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_OPEN日D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_OPEN日D.Enter
		'入力チェック
	...
	Private Sub tx_OPEN日D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_OPEN日D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		''''    If KeyAscii <> 8 Then ' バックスペースは例外
		''''        If LenB(StrConv([tx_OPEN日D].Text & Chr$(KeyAscii), vbFromUnicode)) - _
		'''''            LenB(StrConv([tx_OPEN日D].SelText, vbFromUnicode)) = [tx_OPEN日D].MaxLength Then
		''''            If KeyAscii <> 0 Then       '入力キーがエラーでないならば
		''''                    ReturnF = True
		''''                    [tx_OPEN日D].SetFocus
		''''            End If
	...
	Private Sub tx_OPEN日D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_OPEN日D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_OPEN日D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_物件種別_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_物件種別.Enter
		...
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "物件種別を入力して下さい。"
		PreviousControl = Me.ActiveControl
...
EventExitSub: 
	...
	'2015/10/16 ADD↓
	Private Sub tx_受付日付Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受付日付Y.Enter
		'入力チェック
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "受付日付を入力して下さい。"
	End Sub
	
	Private Sub tx_受付日付Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受付日付Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_受付日付Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_受付日付Y].SelectedText, vbFromUnicode)) = [tx_受付日付Y].Maxlength Then
				ReturnF = True
				[tx_受付日付Y].Focus()
			End If
		End If
	...
	Private Sub tx_受付日付Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受付日付Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受付日付Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_受付日付M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受付日付M.Enter
		'入力チェック
	...
	Private Sub tx_受付日付M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受付日付M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_受付日付M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_受付日付M].SelectedText, vbFromUnicode)) = [tx_受付日付M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_受付日付M].Focus()
				End If
	...
	Private Sub tx_受付日付M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受付日付M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受付日付M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_受付日付D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受付日付D.Enter
		'入力チェック
	...
	Private Sub tx_受付日付D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受付日付D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'nop
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
	...
	Private Sub tx_受付日付D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受付日付D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受付日付D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_完工日付Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完工日付Y.Enter
		'入力チェック
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "完工日付を入力して下さい。"
	End Sub
	
	Private Sub tx_完工日付Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_完工日付Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_完工日付Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_完工日付Y].SelectedText, vbFromUnicode)) = [tx_完工日付Y].Maxlength Then
				ReturnF = True
				[tx_完工日付Y].Focus()
			End If
		End If
	...
	Private Sub tx_完工日付Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完工日付Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完工日付Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_完工日付M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完工日付M.Enter
		'入力チェック
	...
	Private Sub tx_完工日付M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_完工日付M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_完工日付M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_完工日付M].SelectedText, vbFromUnicode)) = [tx_完工日付M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_完工日付M].Focus()
				End If
	...
	Private Sub tx_完工日付M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完工日付M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完工日付M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_完工日付D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完工日付D.Enter
		'入力チェック
	...
	Private Sub tx_完工日付D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_完工日付D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'nop
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
	...
	Private Sub tx_完工日付D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完工日付D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完工日付D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_発注担当者名_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_発注担当者名.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "発注担当者名を入力して下さい。"
	End Sub
	
	Private Sub tx_発注担当者名_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_発注担当者名_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_発注担当者名.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_作業内容_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_作業内容.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "作業内容を入力して下さい。"
	End Sub
	
	Private Sub tx_作業内容_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_作業内容_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_作業内容.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2016/06/09 ADD↓
	Private Sub tx_クレーム区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_クレーム区分.Enter
		If Item_Check(([tx_クレーム区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "クレーム区分を入力して下さい。1:クレーム"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_クレーム区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_クレーム区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_クレーム区分].Text, vbFromUnicode)) = [tx_クレーム区分].Maxlength - 1 Then
					Call SelText([tx_クレーム区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_クレーム区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_クレーム区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_クレーム区分].SelectedText, vbFromUnicode)) = [tx_クレーム区分].Maxlength - 1 Then
					ReturnF = True
					[tx_クレーム区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_クレーム区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_クレーム区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_クレーム区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2015/02/04 ADD↓
	Private Sub tx_伝票種類_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_伝票種類.Enter
		If Item_Check(([tx_伝票種類].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "伝票種類を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_伝票種類_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_伝票種類.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_伝票種類].Text, vbFromUnicode)) = [tx_伝票種類].Maxlength - 1 Then
					Call SelText([tx_伝票種類])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_伝票種類.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_伝票種類].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_伝票種類].SelectedText, vbFromUnicode)) = [tx_伝票種類].Maxlength - 1 Then
					ReturnF = True
					[tx_伝票種類].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_伝票種類_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_伝票種類_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_伝票種類.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2015/11/19 ADD↓
	Private Sub tx_SM内容区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_SM内容区分.Enter
		If Item_Check(([tx_SM内容区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "内容区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_SM内容区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_SM内容区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_SM内容区分].Text, vbFromUnicode)) = [tx_SM内容区分].Maxlength - 1 Then
					Call SelText([tx_SM内容区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_SM内容区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_SM内容区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_SM内容区分].SelectedText, vbFromUnicode)) = [tx_SM内容区分].Maxlength - 1 Then
					ReturnF = True
					[tx_SM内容区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_SM内容区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_SM内容区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_SM内容区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'''            Exit Sub
	'''        Else
	'''            If LenB(StrConv([tx_見積区分].Text, vbFromUnicode)) = [tx_見積区分].MaxLength - 1 Then
	'''                Call SelText([tx_見積区分])
	'''            End If
	'''            If LenB(StrConv([tx_見積区分].Text & Chr$(KeyAscii), vbFromUnicode)) - _
	''''                LenB(StrConv([tx_見積区分].SelText, vbFromUnicode)) = [tx_見積区分].MaxLength - 1 Then
	'''                ReturnF = True
	'''                [tx_見積区分].SetFocus
	'''            End If
	'''        End If
	...
	'2015/06/12 ADD↓
	Private Sub tx_ウエルシア物件内容CD_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア物件内容CD.Enter
		If SelectF = False Then
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ウエルシア物件内容CDを入力して下さい。　選択画面：Space"
	End Sub
	
	Private Sub tx_ウエルシア物件内容CD_SpcKeyPress(ByRef KeyAscii As Short, ByRef Cancel As Boolean)
		If KeyAscii = Asc(" ") And ([tx_ウエルシア物件内容CD].SelectionStart = 0 And [tx_ウエルシア物件内容CD].SelectionLength = Len([tx_ウエルシア物件内容CD].Text)) Then
			KeyAscii = 0
			'---参照画面表示
			SelectF = True
			'---参照画面表示
	...
	Private Sub tx_ウエルシア物件内容CD_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_ウエルシア物件内容CD_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア物件内容CD.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_ウエルシア物件内容CD.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_ウエルシア物件内容名_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア物件内容名.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ウエルシア物件内容名を入力して下さい。"
	End Sub
	
	Private Sub tx_ウエルシア物件内容名_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_ウエルシア物件内容名_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア物件内容名.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_ウエルシア売場面積_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア売場面積.Enter
		If Item_Check(([tx_ウエルシア売場面積].TabIndex)) = False Then
			Exit Sub
		End If
		
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ウエルシア売場面積を入力して下さい。"
		PreviousControl = Me.ActiveControl
	End Sub
	
	Private Sub tx_ウエルシア売場面積_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_ウエルシア売場面積_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア売場面積.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_ウエルシアリース区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシアリース区分.Enter
		...
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ウエルシアリース区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
...
EventExitSub: 
	...
	Private Sub tx_ウエルシア物件区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_ウエルシア物件区分.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "ウエルシア物件区分を入力して下さい。　選択画面：Space"
	End Sub
	...
	'2015/11/03 ADD↓
	Private Sub tx_化粧品メーカー区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_化粧品メーカー区分.Enter
		If Item_Check(([tx_化粧品メーカー区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "化粧品メーカー区分を入力して下さい。（1:在庫計算対象外とする）"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_化粧品メーカー区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_化粧品メーカー区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_化粧品メーカー区分].Text, vbFromUnicode)) = [tx_化粧品メーカー区分].Maxlength - 1 Then
					Call SelText([tx_化粧品メーカー区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_化粧品メーカー区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_化粧品メーカー区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_化粧品メーカー区分].SelectedText, vbFromUnicode)) = [tx_化粧品メーカー区分].Maxlength - 1 Then
					ReturnF = True
					[tx_化粧品メーカー区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_化粧品メーカー区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_化粧品メーカー区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_化粧品メーカー区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2015/10/16 ADD↓
	Private Sub tx_YKサプライ区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_YKサプライ区分.Enter
		If Item_Check(([tx_YKサプライ区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "YKサプライ区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_YKサプライ区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_YKサプライ区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_YKサプライ区分].Text, vbFromUnicode)) = [tx_YKサプライ区分].Maxlength - 1 Then
					Call SelText([tx_YKサプライ区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_YKサプライ区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_YKサプライ区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_YKサプライ区分].SelectedText, vbFromUnicode)) = [tx_YKサプライ区分].Maxlength - 1 Then
					ReturnF = True
					[tx_YKサプライ区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_YKサプライ区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_YKサプライ区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_YKサプライ区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_YK物件区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_YK物件区分.Enter
		If Item_Check(([tx_YK物件区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "YK物件区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_YK物件区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_YK物件区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_YK物件区分].Text, vbFromUnicode)) = [tx_YK物件区分].Maxlength - 1 Then
					Call SelText([tx_YK物件区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_YK物件区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_YK物件区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_YK物件区分].SelectedText, vbFromUnicode)) = [tx_YK物件区分].Maxlength - 1 Then
					ReturnF = True
					[tx_YK物件区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_YK物件区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_YK物件区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_YK物件区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_YK請求区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_YK請求区分.Enter
		If Item_Check(([tx_YK請求区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "YK請求区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_YK請求区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_YK請求区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_YK請求区分].Text, vbFromUnicode)) = [tx_YK請求区分].Maxlength - 1 Then
					Call SelText([tx_YK請求区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_YK請求区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_YK請求区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_YK請求区分].SelectedText, vbFromUnicode)) = [tx_YK請求区分].Maxlength - 1 Then
					ReturnF = True
					[tx_YK請求区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_YK請求区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_YK請求区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_YK請求区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	'2015/10/16 ADD↑
	'2022/10/10 ADD↓
	Private Sub tx_B請求管轄区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_B請求管轄区分.Enter
		If Item_Check(([tx_B請求管轄区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "B請求管轄区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_B請求管轄区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_B請求管轄区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_B請求管轄区分].Text, vbFromUnicode)) = [tx_B請求管轄区分].Maxlength - 1 Then
					Call SelText([tx_B請求管轄区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_B請求管轄区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_B請求管轄区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_B請求管轄区分].SelectedText, vbFromUnicode)) = [tx_B請求管轄区分].Maxlength - 1 Then
					ReturnF = True
					[tx_B請求管轄区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_B請求管轄区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_B請求管轄区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_B請求管轄区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_BtoB番号_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_BtoB番号.Enter
		If Item_Check(([tx_BtoB番号].TabIndex)) = False Then
			Exit Sub
		End If
		
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "BtoB番号を入力して下さい。"
		PreviousControl = Me.ActiveControl
	End Sub
	
	Private Sub tx_BtoB番号_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_BtoB番号_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_BtoB番号.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_現場名_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_現場名.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "現場名を入力して下さい。"
	End Sub
	
	Private Sub tx_現場名_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_現場名_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_現場名.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_支払条件_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_支払条件.Enter
		If Item_Check(([tx_支払条件].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "御支払条件を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_支払条件_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_支払条件.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_支払条件].Text, vbFromUnicode)) = [tx_支払条件].Maxlength - 1 Then
					Call SelText([tx_支払条件])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_支払条件.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_支払条件].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_支払条件].SelectedText, vbFromUnicode)) = [tx_支払条件].Maxlength - 1 Then
					ReturnF = True
					[tx_支払条件].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_支払条件_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_支払条件_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_支払条件.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_支払条件他_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_支払条件他.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "支払条件内容を入力して下さい。"
	End Sub
	
	Private Sub tx_支払条件他_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_支払条件他_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_支払条件他.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納期表示_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納期表示.Enter
		If Item_Check(([tx_納期表示].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "納期表示を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_納期表示_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_納期表示.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_納期表示].Text, vbFromUnicode)) = [tx_納期表示].Maxlength - 1 Then
					Call SelText([tx_納期表示])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_納期表示.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_納期表示].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_納期表示].SelectedText, vbFromUnicode)) = [tx_納期表示].Maxlength - 1 Then
					ReturnF = True
					[tx_納期表示].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_納期表示_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納期表示_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納期表示.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_納期表示他_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納期表示他.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "納期内容を入力して下さい。"
	End Sub
	
	Private Sub tx_納期表示他_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_納期表示他_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_納期表示他.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_出力日_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_出力日.Enter
		If Item_Check(([tx_出力日].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "見積日出力を選択して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_出力日_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_出力日.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_出力日].Text, vbFromUnicode)) = [tx_出力日].Maxlength - 1 Then
					Call SelText([tx_出力日])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_出力日.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_出力日].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_出力日].SelectedText, vbFromUnicode)) = [tx_出力日].Maxlength - 1 Then
					ReturnF = True
					[tx_出力日].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_出力日_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_出力日_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_出力日.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_有効期限_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_有効期限.Enter
		If Item_Check(([tx_有効期限].TabIndex)) = False Then
			Exit Sub
		End If
		
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "有効期限を入力して下さい。"
		PreviousControl = Me.ActiveControl
	End Sub
	
	Private Sub tx_有効期限_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_有効期限_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_有効期限.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_受注区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注区分.Enter
		If Item_Check(([tx_出力日].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "受注区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_受注区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受注区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_受注区分].Text, vbFromUnicode)) = [tx_受注区分].Maxlength - 1 Then
					Call SelText([tx_受注区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_受注区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_受注区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_受注区分].SelectedText, vbFromUnicode)) = [tx_受注区分].Maxlength - 1 Then
					ReturnF = True
					[tx_受注区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_受注区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受注区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_受注区分.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_受注日付Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注日付Y.Enter
		'入力チェック
		If Item_Check(([tx_受注日付Y].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "受注日付を入力して下さい。"
		PreviousControl = Me.ActiveControl
		'ボタン名設定
		'UPGRADE_ISSUE: Control Name は、汎用名前空間 ActiveControl 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
		Call SetUpFuncs(Me.ActiveControl.Name)
	...
	Private Sub tx_受注日付Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受注日付Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_受注日付Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_受注日付Y].SelectedText, vbFromUnicode)) = [tx_受注日付Y].Maxlength Then
				ReturnF = True
				[tx_受注日付Y].Focus()
			End If
		End If
	...
	Private Sub tx_受注日付Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受注日付Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注日付Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_受注日付Y.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_受注日付M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注日付M.Enter
		'入力チェック
	...
	Private Sub tx_受注日付M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受注日付M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_受注日付M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_受注日付M].SelectedText, vbFromUnicode)) = [tx_受注日付M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_受注日付M].Focus()
				End If
	...
	Private Sub tx_受注日付M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受注日付M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注日付M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_受注日付M.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_受注日付D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注日付D.Enter
		'入力チェック
	...
	Private Sub tx_受注日付D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_受注日付D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		''''    If KeyAscii <> 8 Then ' バックスペースは例外
		''''        If LenB(StrConv([tx_受注日付D].Text & Chr$(KeyAscii), vbFromUnicode)) - _
		'''''            LenB(StrConv([tx_受注日付D].SelText, vbFromUnicode)) = [tx_受注日付D].MaxLength Then
		''''            If KeyAscii <> 0 Then       '入力キーがエラーでないならば
		''''                    ReturnF = True
		''''                    [tx_受注日付D].SetFocus
		''''            End If
	...
	Private Sub tx_受注日付D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_受注日付D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_受注日付D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
			'UPGRADE_WARNING: オブジェクト tx_受注日付D.Undo の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Sub tx_大小口区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_大小口区分.Enter
		If Item_Check(([tx_大小口区分].TabIndex)) = False Then
			Exit Sub
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "大小口区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
	...
	Private Sub tx_大小口区分_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_大小口区分.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'MaxLengthは１多く設定する。
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			Else
				...
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_大小口区分].Text, vbFromUnicode)) = [tx_大小口区分].Maxlength - 1 Then
					Call SelText([tx_大小口区分])
				End If
				'UPGRADE_WARNING: TextBox プロパティ tx_大小口区分.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
				'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
				'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
				If LenB(StrConv([tx_大小口区分].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_大小口区分].SelectedText, vbFromUnicode)) = [tx_大小口区分].Maxlength - 1 Then
					ReturnF = True
					[tx_大小口区分].Focus()
				End If
			End If
...
EventExitSub: 
	...
	Private Sub tx_大小口区分_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_大小口区分_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_大小口区分.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_出精値引_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_出精値引.Enter
		If Item_Check(([tx_出精値引].TabIndex)) = False Then
			Exit Sub
		End If
		
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "出精値引(マイナス)を入力して下さい。"
		PreviousControl = Me.ActiveControl
	End Sub
	
	Private Sub tx_出精値引_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_出精値引_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_出精値引.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_見積確定区分_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_見積確定区分.Enter
		...
		End If
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "見積確定区分を入力して下さい。"
		PreviousControl = Me.ActiveControl
...
EventExitSub: 
	...
	Private Sub tx_完了日Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了日Y.Enter
		'入力チェック
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "完了日を入力して下さい。"
	End Sub
	
	Private Sub tx_完了日Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_完了日Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_完了日Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_完了日Y].SelectedText, vbFromUnicode)) = [tx_完了日Y].Maxlength Then
				ReturnF = True
				[tx_完了日Y].Focus()
			End If
		End If
	...
	Private Sub tx_完了日Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完了日Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了日Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_完了日M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了日M.Enter
		'入力チェック
	...
	Private Sub tx_完了日M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_完了日M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_完了日M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_完了日M].SelectedText, vbFromUnicode)) = [tx_完了日M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_完了日M].Focus()
				End If
	...
	Private Sub tx_完了日M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完了日M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了日M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_完了日D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了日D.Enter
		'入力チェック
	...
	Private Sub tx_完了日D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_完了日D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'nop
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
	...
	Private Sub tx_完了日D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完了日D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了日D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_完了者名_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了者名.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "完了者名を入力して下さい。"
	End Sub
	
	Private Sub tx_完了者名_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_完了者名_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_完了者名.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_請求予定Y_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_請求予定Y.Enter
		'入力チェック
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "請求予定を入力して下さい。"
	End Sub
	
	Private Sub tx_請求予定Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_請求予定Y.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_請求予定Y].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_請求予定Y].SelectedText, vbFromUnicode)) = [tx_請求予定Y].Maxlength Then
				ReturnF = True
				[tx_請求予定Y].Focus()
			End If
		End If
	...
	Private Sub tx_請求予定Y_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_請求予定Y_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_請求予定Y.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_請求予定M_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_請求予定M.Enter
		'入力チェック
	...
	Private Sub tx_請求予定M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_請求予定M.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB(StrConv([tx_請求予定M].Text & Chr(KeyAscii), vbFromUnicode)) - LenB(StrConv([tx_請求予定M].SelectedText, vbFromUnicode)) = [tx_請求予定M].Maxlength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					ReturnF = True
					[tx_請求予定M].Focus()
				End If
	...
	Private Sub tx_請求予定M_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_請求予定M_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_請求予定M.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_請求予定D_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_請求予定D.Enter
		'入力チェック
	...
	Private Sub tx_請求予定D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_請求予定D.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		'nop
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
	...
	Private Sub tx_請求予定D_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_請求予定D_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_請求予定D.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_経過備考1_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_経過備考1.Enter
		...
		Call SetUpFuncs(Me.ActiveControl.Name)
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = "備考を入力して下さい。最大全角23文字"
	End Sub
	
	Private Sub tx_経過備考1_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_経過備考1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_経過備考1.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Sub tx_経過備考2_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_経過備考2.Enter
	...
	Private Sub tx_経過備考2_RtnKeyDown(ByRef KeyCode As Short, ByRef Shift As Short, ByRef Cancel As Boolean)
		ReturnF = True
	End Sub
	
	Private Sub tx_経過備考2_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tx_経過備考2.Leave
		'UPGRADE_WARNING: コレクション sb_Msg.Panels の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
		[sb_Msg].Items.Item(1).Text = ""
		If ReturnF = False Then
	...
	Private Function Upload() As Boolean
		...
		'UPGRADE_WARNING: オブジェクト SpcToNull() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		'UPGRADE_WARNING: オブジェクト HD_得意先別見積番号 の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		HD_得意先別見積番号 = SpcToNull(rf_得意先別見積番号) '2005/07/04 ADD
		...
		'2020/04/11 ADD↓
		'UPGRADE_WARNING: オブジェクト SpcToNull() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		HD_見積確定区分 = SpcToNull(([tx_見積確定区分].Text), 0)
	...
	Private Sub Purge()
		...
		With cmd.Parameters
			'UPGRADE_WARNING: オブジェクト SpcToNull() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			.Item(1).Value = SpcToNull([rf_見積番号])
		End With
