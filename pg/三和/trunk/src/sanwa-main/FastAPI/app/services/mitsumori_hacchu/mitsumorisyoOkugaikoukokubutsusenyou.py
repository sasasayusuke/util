...
from app.utils.service_utils import ClsTanto
...
class MitsumorisyoOkugaikoukokubutsusenyouService(BaseService):
    def display(self, request, session):
        """ 見積書出力処理 (index) """
        logger.info("【START】見積書出力処理")

        # ストアプロシージャだし分け
        stored_name = ""
        if request.params["書式区分"] in ["0", "1"]:
            stored_name = "usp_MT0300見積書出力"
        elif request.params["書式区分"] == "2":
            stored_name = "usp_MT0301見積書出力_客在"
        elif request.params["書式区分"] == "3":
            stored_name = "usp_MT0302見積書出力_部材計"
        elif request.params["書式区分"] == "4":
            stored_name = "usp_MT0303見積書出力_部材計_客在"
        else:
            raise ServiceError('書式区分が入力されていません')

        # ストアプロシージャから取得為の必須条件
        params = {"@i見積番号": request.params.get('@i見積番号')}
        ...
        with get_excel_buffer() as buffer:
            # テンプレートファイル
            wb = create_excel_object('Template_見積書(屋外広告物専用).xlsx')
            # テンプレートシートのコピー
            template_sheet = wb['別紙_テンプレート']
            new_sheet = wb.copy_worksheet(template_sheet)
            copy_ws = wb['Sheet1']
            # コピー先シート名
            new_sheet.title = "見積積算書"
            # ページ設定の引き継ぎ
            ...
            # データ量を判定して出力処理を振り分け
            if storedresults["count"] < 16:  # データ数が16より少ない場合
    ...
    def populate_data_to_sheet(self,session, ws,copy_ws, array, request, min_mode=False):
        ...
        if min_mode:
            ...
            # データの挿入処理
            self.insert_third_array_data(ws,copy_ws, array[2], 22, 1,request)
            ...
            # 第二配列のデータ挿入
            current_row = self.insert_second_array_data(ws, array[1], 22, 1,request)
    ...
    def insert_static_data_to_sheet(self,session, ws, array, request):
        ...
        # 御見積金額
        if request.params["書式区分"] != "1":
            合計金額 = float(data.get("合計金額", 0))
            出精値引 = float(data.get("出精値引", 0))
            ws["D8"].value = 合計金額 + 出精値引
            ws["D10"].value = data.get("外税額", "")
        else:
            ws['D11'].value = ""

        # 見積件名
        ws["D13"].value = data.get("見積件名", "")
        ...
        if 納期表示 == 0:
            if 納期S is None and 納期E is None:
                ...
                ws["D14"].value = f"{format_jp_date(納期S)}～{format_jp_date(納期E)}"
            elif 納期S is None:
                ws["D14"].value = format_jp_date(納期E)
            else:
                ws["D14"].value = format_jp_date(納期S)
        elif 納期表示 == 1:
            ws["D14"].value = "別途御打ち合せによる"
        elif 納期表示 == 2:
            ws["D14"].value = 納期その他
        ...
        if sales_area:
            ws["A12"].value = "売 場 面 積 ："
            ws["D12"].value = f"{sales_area}坪"
    ...
    def insert_second_array_data(self, ws, second_array, start_row, start_col,request):
        """
        ...
        for item in second_array:
            # 各データを列に挿入
            ws.cell(row=current_row, column=start_col).value = item.get("仕分番号", "")  # 列1のデータ
            ws.cell(row=current_row, column=start_col +2).value = item.get("仕分名称", "")  # 列2のデータ
            if request.params["書式区分"] != '1':
                ws.cell(row=current_row, column=start_col +14).value = item.get("売上金額", "")  # 列3のデータ
    ...
    def insert_third_array_data(self, ws,copy_ws, third_array, start_row, start_col,request):
        ...
        if sorted_third_array:
            ws.cell(row=start_row, column=start_col).value = self.get_value_or_empty(
                sorted_third_array[0], "仕分番号")  # 仕分名称
            ws.cell(row=start_row, column=start_col +
                    2).value = self.get_value_or_empty(sorted_third_array[0], "仕分名称")  # 仕分名称

        # ソートしたデータをループで挿入
        for item in sorted_third_array:
            # 1行分のデータを挿入するためにinsert_row_data関数を利用
            self.insert_row_data(ws,copy_ws, current_row, start_col, item,request)
    ...
    def insert_third_array_group_data(self, ws, copy_ws,array, start_row, start_col,request):
        ...
        def insert_detail(ws,copy_ws,row):
            row = ((page_count-1) * table_height) + 37
    ...
    def insert_row_data(self, ws,copy_ws, row, col, item,request):
        """1行分のデータをExcelシートに挿入"""
        # 変数定義
        ...
        if request.params["書式区分"] != "1":
            ...
            if not getQuoteKubun in ['A','C','S']:
                    self.write_cell(ws, row, col + 14, self.get_value_or_empty(item, "売上金額"))
            else:
                if "\\" in null_to_zero(item['漢字名称'],""):
                    self.write_cell(ws, row, col + 9, 1)
                    self.write_cell(ws, row, col + 11, "式")
                    tmp = null_to_zero(item["漢字名称"],"")
                    position = tmp.find("\\")
                    tmp = tmp[position:]
                    self.write_cell(ws, row, col + 12, int(tmp.replace("\\","").replace(",","")))
                    self.write_cell(ws, row, col + 14, int(tmp.replace("\\","").replace(",","")))
                    ws['D9'].value = int(tmp.replace("\\","").replace(",",""))
                else:
                    ws.cell(row=row,column=15,value="" if item["売上金額"] == 0 else item["売上金額"])
