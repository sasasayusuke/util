...
class kensyuitiranhyoService(BaseService):
    """検収一覧表サービス"""

    def display(self, request, session) -> StreamingResponse:

        # 検収一覧表の印刷処理
        logger.info("【START】検収一覧表処理")

        storedname="usp_SH0200検収一覧表出力"

        params = {
            "@i支払日付": request.params['@i支払日付'],
        ...
        with get_excel_buffer() as buffer:
            wb = create_excel_object('Template_検収一覧表.xlsx')
            ws = wb["Template"]
            ts = wb["Copy"]

            paymentDate = request.params["@i支払日付"]
            supplierCodeFrom = request.params["@i開始仕入先"]
            supplierCodeCodeTo = request.params["@i終了仕入先"]

            # ヘッダー設定
            if request.params['@締日'] == '99':
                ws["E1"] = str(paymentDate)[:4] + '年' + str(paymentDate)[5:7] + '月末締'
            else:
                ws["E1"] = str(paymentDate)[:4] + '年' + str(paymentDate)[5:7] + '月' + str(paymentDate)[8:10] + '日締'
            ...
            for result in storedresults['results'][0]:
                ...
                ws.cell(row=current_row+1, column=19, value=result['支払金額'])
                ws.cell(row=current_row+1, column=23, value=result['調整金額'])
                ws.cell(row=current_row+1, column=27, value=result['差引繰越額'])
                ws.cell(row=current_row+1, column=31, value=result['仕入金額'])
                ws.cell(row=current_row+1, column=35, value=result['返品金額'])
                ws.cell(row=current_row+1, column=39, value=result['訂正金額'])
                ws.cell(row=current_row+1, column=43, value=result['差引仕入額'])
                ws.cell(row=current_row+1, column=47, value=result['消費税額'])
                ws.cell(row=current_row+1, column=51, value=result['税込仕入額'])
                ws.cell(row=current_row+1, column=55, value=result['当月残高'])
                ws.cell(row=current_row, column=59, value=result['支払予定日'])
                ws.cell(row=current_row+1, column=64, value=result['支払様式名'])

                # 総合計に加算
                sum_list[0] += result['前月残高']
                sum_list[1] += result['支払金額']
                sum_list[2] += result['調整金額']
                sum_list[3] += result['差引繰越額']
                sum_list[4] += result['仕入金額']
                sum_list[5] += result['返品金額']
                sum_list[6] += result['訂正金額']
                sum_list[7] += result['差引仕入額']
                sum_list[8] += result['消費税額']
                sum_list[9] += result['税込仕入額']
                sum_list[10] += result['当月残高']
            ...
            ws.cell(row=current_row+1, column=55, value=sum_list[10])
