...
async function create_searchDialog(dialogId,tableId,fields,btnFields = [],width = '750px',lastSql = ""){
    if (document.getElementById(dialogId)) {
        let message = `[SYS-062] ダイアログID "${dialogId}" は既に存在します。一意のIDを使用してください。`;
        alert(message)
    ...
    for(i = 0; i < 2; i++){
        ...
        for(item of searchTable_items){
            let tmp_searchObj = {dialogId:dialogId,searchTableId:tableId,tableId:item.id,width:width,lastSql:lastSql,allCheck:item.options.allCheck ? true:false};
            // toColumns作成
...
// ③ダイアログを表示する
$(document).on('click','.dialogSearch',async function(){
    ...
    // ロック項目だった場合、画面クリア
    const lock_id = $(this).prev().attr('id');
    const target_lockItem = lockItems.find(e => `${e["dialogId"]}_${e["id"]}` == id);
    if(target_lockItem){
        // 前の項目にfocusを戻し、画面を初期化（請求書系）
        // $(this).prev().focus();
        $(`#${id}`).focus();
        // 虫眼鏡ボタンにfocusを戻す
        // ※focusを戻さないと検索ダイアログを閉じた際にfocusが見積番号に入り、クリア処理が走ってしまう為
        $(this).focus();
        // await sleep(0);
    }
...
function openDialog_for_searchDialog(dialogId,id,width,title){
    ...
    $(`#${dialogId}Dialog`).dialog('destroy');
    $(`#${dialogId}Dialog`).dialog({
        modal: true,
        width: width,
        resizable: true,
...
async function create_searchList(event){
    ...
    let arr = [];
    // 簡単なSQL・joinなど複雑なSQLで取得できる処理で分け
    ...
    for(item of arr){
        if(searchDialogData.multiple  || searchDialogData.dialogId.includes('_multiple')){
            html += `<tr tabindex="0"><td class="searchTd bgc"><input type="checkbox" ${searchDialogData.allCheck ? 'checked':''}></td>`;
        }else{
            html += '<tr tabindex="0">';
        }
...
const createWhereParam = (dialogData) =>{
    ...
    dialogData.options.forEach(e =>{
        ...
        if((e.type == 'range-text' && e.type != 'range-number')/* && "alignment" in e*/){
        ...
        else if(e.type == 'over-text'){
            tmp[e.searchColumn] = {from:$(`#${fullId}`).val(),to:''}
        }
        else{
...
async function requestSQL(dialogData){
    ...
    let lastSql = dialogData.lastSql;

    let sql = `SELECT TOP 1000 ${select} FROM ${from} ${where} ${lastSql}`;
...
async function requestIndividualSQL(dialogData){
    ...
    else if(['kaikakeKakuninSearch'].includes(dialogData.dialogId)){
        sql = kaikakeKakunin_baseSQL(where).replaceAll('\n',' ').replaceAll(/\s{2,}/g, ' ');
    }
    ...
    // 売上前請求
    else if(['estimateNoSearch6','estimateNoSearch6_multiple'].includes(dialogData.dialogId)){
        sql = estimateNoSearch6_baseSQL(where).replaceAll('\n',' ').replaceAll(/\s{2,}/g, ' ');
    }

    // 補助科目検索ダイアログ
    else if(['auxiliarySubjectSearch','auxiliarySubjectSearch_multiple'].includes(dialogData.dialogId)){
        sql = auxiliarySubjectSearch_baseSQL(where).replaceAll('\n',' ').replaceAll(/\s{2,}/g, ' ');
    }
    // 請求原価率表
    else if("invoiceCost" == dialogData.dialogId){
        sql = invoiceCost_baseSQL(where).replaceAll('\n',' ').replaceAll(/\s{2,}/g, ' ');
    }
...
// 選択されたときの処理
$(document).on('click','tr:has(".searchTd")',async function(e){
    ...
    if(searchOutputId[searchOutputId.length - 1].id == "changeProductDialog_productNo"){
        ...
        // 応急的に見積Noの場合は0埋めをする
        let tmp = item.children()[0].textContent;
        $(`#${searchOutputId[searchOutputId.length -1].id}`).val(tmp).trigger('blur');
    }

    $(`#${searchOutputId[searchOutputId.length -1].dialogId}Dialog tbody`).html('');
    closeDialog(`${searchOutputId[searchOutputId.length -1].dialogId}Dialog`,false);
})
...
// 
// 検索系ダイアログはescape押下時に閉じる
$(document).keydown(async function(e){
    if(searchOutputId.length == 0)return;
    const key = e.keyCode;
    if(key != 27){
        return;
    }
    ...
    const dialogId = searchOutputId[searchOutputId.length -1].dialogId + "Dialog";
