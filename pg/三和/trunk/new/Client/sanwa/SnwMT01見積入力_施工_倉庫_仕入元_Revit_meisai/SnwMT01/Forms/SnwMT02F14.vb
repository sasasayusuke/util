...
Friend Class SnwMT02F14
	...
	'表示項目をセット
	WriteOnly Property FpSpd() As FpSpread
	...
	Private Sub SnwMT02F14_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		'フォームを画面の中央に配置
	...
	Private Sub SnwMT02F14_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
...
Update_Err: '---エラー時
		...
		For i = 1 To cTable.Count()
			With rs
				...
				If cTable.Item(i)(16) <> "削" Then
					'            [sb_Msg].Items.Item(0).Text = cTable(i)(1) & cTable(i)(2) & cTable(i)(3) & "," & cTable(i)(6) & "," & cTable(i)(11) & "," & cTable(i)(5) & "," & cTable(i)(25)
	...
	Private Sub Cb変更_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Cb変更.Click
		...
		If ShowOpenFileDialog(GXLSPath, ProfileKey, "CSV") = True Then
...
CreateTable_Err:
	...
	Private Function UpdateSheet() As Short
		...
		Do Until gRsA.EOF
			'2017/10/05 ADD↓
			If clsSPD.GetTextEX(Col見積区分, pActRow + Mcnt) = "" And clsSPD.GetTextEX(Col製品NO, pActRow + Mcnt) = "" And clsSPD.GetTextEX(Col仕様NO, pActRow + Mcnt) = "" And clsSPD.GetTextEX(Col名称, pActRow + Mcnt) = "" Then
				...
				'明細部作成
				With pfpSpd
					.ActiveSheet.SetText(pActRow + Mcnt, Col他社納品日付, gRsA.Fields("納品日").Value)
					.ActiveSheet.SetText(pActRow + Mcnt, Col他社伝票番号, gRsA.Fields("伝票番号").Value)
					.ActiveSheet.SetText(pActRow + Mcnt, Col製品NO, gRsA.Fields("単品コード").Value)

					.ActiveSheet.SetText(pActRow + Mcnt, Col仕様NO, "S") '2016/08/03 ADD

					'クリアする 2016/08/03 ADD
					.ActiveSheet.SetText(pActRow + Mcnt, Col名称, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColW, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColD, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColH, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColD1, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColD2, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColH1, "")
					.ActiveSheet.SetText(pActRow + Mcnt, ColH2, "")
					.ActiveSheet.SetText(pActRow + Mcnt, Col原価, "")
					.ActiveSheet.SetText(pActRow + Mcnt, Col売価, "")
					.ActiveSheet.SetText(pActRow + Mcnt, Col単位, "")
					'            .SetText Col仕入先CD, pActRow + Mcnt, ""
					'            .SetText Col仕入先名, pActRow + Mcnt, ""
					.ActiveSheet.SetText(pActRow + Mcnt, Col仕入先CD, "3150") '2017/01/06 ADD
					.ActiveSheet.SetText(pActRow + Mcnt, Col仕入先名, SirName) '2017/01/06 ADD
					'            .SetText Col送り先CD, pActRow + Mcnt, "01" '直送 '2016/08/03 ADD
					'            .SetText Col送り先名, pActRow + Mcnt, HaisoName '直送 '2016/08/03 ADD
					.ActiveSheet.SetText(pActRow + Mcnt, Col送り先CD, "03") '三和納品 '2017/01/06 ADD
					.ActiveSheet.SetText(pActRow + Mcnt, Col送り先名, HaisoName) '2017/01/06 ADD

					'            .SetText Col社内在庫, pActRow + Mcnt, gRsA![数量]
					.ActiveSheet.SetText(pActRow + Mcnt, Col仕分数1, gRsA.Fields("数量").Value)
