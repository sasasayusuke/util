...
Friend Class SnwMT03F00S
	...
	Private Sub SnwMT03F00S_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "見積番号", CInt(VB6Conv.TwipsToPixelsX(1040)))
			.Columns.Add("", "見積日付", CInt(VB6Conv.TwipsToPixelsX(1040)))
			.Columns.Add("", "種別", CInt(VB6Conv.TwipsToPixelsX(800)))
			...
			.Columns.Add("", "出力日", CInt(VB6Conv.TwipsToPixelsX(900)))
			.Columns.Add("", "発注日", CInt(VB6Conv.TwipsToPixelsX(900)))
		...
		'UPGRADE_ISSUE: Form プロパティ SnwMT03F00S.ScaleWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
...
Form_Load_Err:
	...
	'UPGRADE_WARNING: イベント SnwMT03F00S.Resize は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub SnwMT03F00S_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub SnwMT03F00S_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
...
EventExitSub:
	...
	Private Sub SnwMT03F00S_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
		...
		'ResultCodeSetControl.Dispose()
		'UPGRADE_NOTE: オブジェクト SnwMT03F00S をガベージ コレクトするまでこのオブジェクトを破棄することはできません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"' をクリックしてください。
		'Me = Nothing
		Me.Dispose()
		e.Cancel = Cancel
	End Sub
		...
		If SelListVw.Items.Count <> 0 Then
			...
			If Not (ResultCodeSetControl Is Nothing) Then
				With ResultCodeSetControl
					...
					.Text = Trim(SelListVw.Items.Item(SelListVw.FocusedItem.Index).Text)
					.Tag = "True"
				End With
			End If
		End If
...
Item_Check_Err:
	...
	Private Function Download() As Boolean
		...
		'---2003/12/04.DEL
		'    SQL = "SELECT MT.見積番号, MT.見積日付, MT.物件種別, MT.得意先CD, MT.見積件名," _
		'        & "MT.合計金額, MT.見積書出力日,MT.注文発行日" _
		'        & " FROM TD見積 AS MT"
		'---2003/12/04.ADD
		sql = "SELECT MT.見積番号, MT.見積日付, MT.物件種別, MT.得意先CD, MT.見積件名," & "MT.合計金額 + MT.出精値引 AS 合計金額, MT.見積書出力日,MT.注文発行日" & " FROM TD見積 AS MT"
		'-----------------
		whr = ""
		...
		If IsCheckText([tx_見積件名]) = True Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & "見積件名 Like '%" & SQLString([tx_見積件名].Text) & "%'"
		End If
		'見積金額
		If IsCheckText([tx_s見積金額]) = True Or IsCheckText(tx_e見積金額) = True Then
			...
			End If
			'---2003/12/04.DEL
			'        whr = whr & SQLCurRange("MT.合計金額", Trim$(tx_s見積金額), Trim$(tx_e見積金額), , False)
			'---2003/12/04.ADD
		...
		If whr <> "" Then
			whr = " WHERE " & whr
		End If
		sql = sql & whr & " ORDER BY MT.見積日付 DESC, MT.見積番号 DESC"
		...
		Do Until rs.EOF
			...
			itmX.SubItems.Add(CType(NullToZero((rs.Fields(1).Value), ""), DateTime).ToString("MM/dd"))
			...
			'        itmX.ListSubItems.Add , , ret '2012/09/06 DEL
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CStr(NullToZero((rs.Fields(3).Value), "")))
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CStr(NullToZero((rs.Fields(4).Value), "")))
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CType(NullToZero((rs.Fields(5).Value), ""), Double).ToString("#,##0") & KIN_FMT)
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			If NullToZero(rs.Fields(6).Value, "").ToString = "" Then
				itmX.SubItems.Add("")
			Else
				itmX.SubItems.Add(CType(NullToZero((rs.Fields(6).Value), ""), DateTime).ToString("MM/dd"))
			End If
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			If NullToZero((rs.Fields(7).Value), "").ToString = "" Then
				itmX.SubItems.Add("")
			Else
				itmX.SubItems.Add(CType(NullToZero((rs.Fields(7).Value), ""), DateTime).ToString("MM/dd"))
			End If
			rs.MoveNext()
		Loop
		ReleaseRs(rs)
