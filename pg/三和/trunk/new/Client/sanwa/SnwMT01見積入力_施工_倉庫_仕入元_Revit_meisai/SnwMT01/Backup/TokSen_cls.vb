...
Friend Class TokSen_cls
	...
	''''''       sql         :リストセット用レコードソース
	''''''       Mode        :得意先選択と請求先選択の表示を換える("得意先"・"請求先")
	...
	Private Sub TokSen_cls_FormClosing(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
		Dim Cancel As Boolean = eventArgs.Cancel
		Dim UnloadMode As System.Windows.Forms.CloseReason = eventArgs.CloseReason
		''    Set ResultCodeSetControl = Nothing
		'UPGRADE_NOTE: オブジェクト TokSen_cls をガベージ コレクトするまでこのオブジェクトを破棄することはできません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"' をクリックしてください。
		Me = Nothing
		eventArgs.Cancel = Cancel
	End Sub
	
	'UPGRADE_WARNING: イベント TokSen_cls.Resize は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub TokSen_cls_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
	...
	Private Sub TokSen_cls_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		Dim i, j As Short
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "得意先CD", CInt(VB6.TwipsToPixelsX(1050)))
			.Columns.Add("", "得意先名1", CInt(VB6.TwipsToPixelsX(3500)))
			.Columns.Add("", "得意先名2", CInt(VB6.TwipsToPixelsX(3500)))
		End With
		...
		'UPGRADE_ISSUE: Form プロパティ TokSen_cls.ScaleWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
...
Form_Load_Err: 
	...
	Private Sub TokSen_cls_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
...
Item_Check_Err: 
	...
	Private Function Download() As Boolean
		...
		sql = "SELECT 得意先CD, 得意先名1, 得意先名2 " & "FROM TM得意先 "
		
		If [tx_検索ID].Text <> vbNullString Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & " 得意先CD >= '" & SQLString([tx_検索ID]) & "'"
		End If
		If tx_検索名.Text <> vbNullString Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & " 得意先名1 LIKE '%" & SQLString(tx_検索名) & "%'"
		End If
		...
		Do Until rs.EOF
			Buf = New String("0", TokuIDLength) & "0" 'ひとつ余計にする
