...
Friend Class SnwMT02F05
	...
	Private Sub SnwMT02F05_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
	...
	Private Sub SnwMT02F05_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub SnwMT02F05_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		'Dim i, j As Integer
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "  見積日付", CInt(VB6Conv.TwipsToPixelsX(1300)))
			.Columns.Add("見積番号", CInt(VB6Conv.TwipsToPixelsX(1300)), System.Windows.Forms.HorizontalAlignment.Right)
			.Columns.Add("行番号", CInt(VB6Conv.TwipsToPixelsX(900)), System.Windows.Forms.HorizontalAlignment.Right)
			.Columns.Add("", "見積件名", CInt(VB6Conv.TwipsToPixelsX(4000))) '2006/11/02 ADD
			.Columns.Add("", "名称", CInt(VB6Conv.TwipsToPixelsX(4000)))
			.Columns.Add("", "サイズ", CInt(VB6Conv.TwipsToPixelsX(3000)))
			.Columns.Add("     原価", CInt(VB6Conv.TwipsToPixelsX(1500)), System.Windows.Forms.HorizontalAlignment.Right)
			.Columns.Add("     売価", CInt(VB6Conv.TwipsToPixelsX(1500)), System.Windows.Forms.HorizontalAlignment.Right)
			.Columns.Add("     数量", CInt(VB6Conv.TwipsToPixelsX(1500)), System.Windows.Forms.HorizontalAlignment.Right)
			.Columns.Add("", "単位", CInt(VB6Conv.TwipsToPixelsX(850)))
			.Columns.Add("", "仕入先", CInt(VB6Conv.TwipsToPixelsX(1395))) '2006/11/02 ADD
			'2012/07/02 ADD↓
			.Columns.Add("     原価", CInt(VB6Conv.TwipsToPixelsX(1500)), System.Windows.Forms.HorizontalAlignment.Right)
			.Columns.Add("     売価", CInt(VB6Conv.TwipsToPixelsX(1500)), System.Windows.Forms.HorizontalAlignment.Right)
			'2012/07/02 ADD↑
		...
		SelListVw.Width = VB6Conv.TwipsToPixelsX(14400 + 330) 'スクロールバー分プラス
...
Form_Load_Err:
	...
	Private Sub SnwMT02F05_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
...
CmdCan_Click_Err:
	...
	'Private Sub SelListVw_KeyPress(ByRef KeyAscii As Integer)
...
Item_Check_Err:
	...
	Private Function Download() As Boolean
		...
		Do Until rs.EOF
			...
			Buf = " " & CType(NullToZero((rs.Fields("見積日付").Value), ""), DateTime).ToString("yyyy/MM/dd")
			...
			End If
			'---仕入単価
			itmX.SubItems.Add(CType((rs.Fields("仕入単価").Value), Double).ToString("#,##0.00;-#,##0.00"))
			'        itmX.ListSubItems.Add , , CStr(Format$(rs(3).Value, "#,##0;-#,##0"))
			'---売上単価
			itmX.SubItems.Add(CType((rs.Fields("売上単価").Value), Double).ToString("#,##0.00;-#,##0.00"))
			'        itmX.ListSubItems.Add , , CStr(Format$(rs(4).Value, "#,##0;-#,##0"))
			'---数量
			itmX.SubItems.Add(CType((rs.Fields("見積数量").Value), Double).ToString("#,##0.00;-#,##0.00"))
			'---単位
			...
			'2012/07/02 ADD↓
			'---仕入単価
			itmX.SubItems.Add(CType(NullToZero((rs.Fields("仕入単価2").Value), 0), Double).ToString("#,##0.00;-#,##0.00"))
			'---売上単価
			itmX.SubItems.Add(CType(NullToZero((rs.Fields("売上単価2").Value), 0), Double).ToString("#,##0.00;-#,##0.00"))
			'2012/07/02 ADD↑
			rs.MoveNext()
		Loop
		ReleaseRs(rs)
	...
	Public Sub SelSetup(ByRef GetKBN As Short, ByRef ctl As System.Windows.Forms.Control, ByRef PCKUBN As String, ByRef SEIHNO As String, ByRef SIYONO As String, ByRef TOKUCD As String, ByRef SirCD As String)
	...
	Private Sub tx_見積日付D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_見積日付D.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			If LenB([tx_見積日付D].Text & Chr(KeyAscii)) - LenB([tx_見積日付D].SelText) = [tx_見積日付D].MaxLength Then
				If KeyAscii <> 0 Then '入力キーがエラーでないならば
					'                ReturnF = True
					[tx_見積日付D].Focus()
				End If
