...
'''                              物件種別の区分追加
'''        2003/12/10  kawamura  抽出条件と表示の見積日付→納期S
'''        2004/01/17  oosawa　　納期S・見積番号の降順に並べる
'''                              idcの開放
'''        2012/09/06  oosawa      物件種別をスタテッククラスに変更し名称を持ってくる
'''        2012/09/10  oosawa      得意先CD9999:社内は表示しないように変更
''' --------------------------------------------------------------------
''' </summary>
Public Class SentakMitumori_cls
	...
	'値返還用
	Public m_Parent As clsMitumoriH
	Public m_DialogResult As Boolean
	Public m_DialogResultCode As Integer
	...
	Public WriteOnly Property Parent_Renamed() As clsMitumoriH
		Set(ByVal Value As clsMitumoriH)
			m_Parent = Value
		End Set
	End Property
	...
	Private Sub SentakMitumori_cls_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "見積番号", CInt(VB6Conv.TwipsToPixelsX(1200)))
			.Columns.Add("", "見積日付", CInt(VB6Conv.TwipsToPixelsX(1200)))
			.Columns.Add("", "種別", CInt(VB6Conv.TwipsToPixelsX(1000)))
			.Columns.Add("", "得意先", CInt(VB6Conv.TwipsToPixelsX(900)))
			.Columns.Add("", "見積件名", CInt(VB6Conv.TwipsToPixelsX(3500)))
			.Columns.Add("", "見積金額", CInt(VB6Conv.TwipsToPixelsX(1500)))
			.Columns.Add("", "出力日", CInt(VB6Conv.TwipsToPixelsX(1200)))
			.Columns.Add("", "発注日", CInt(VB6Conv.TwipsToPixelsX(1200)))
			.Columns.Add("", "売上状況", CInt(VB6Conv.TwipsToPixelsX(1200)))
			...
			.Columns.Item(1).TextAlign = System.Windows.Forms.HorizontalAlignment.Center
			'UPGRADE_WARNING: コレクション SelListVw.ColumnHeaders の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
			.Columns.Item(2).TextAlign = System.Windows.Forms.HorizontalAlignment.Center
			'UPGRADE_WARNING: コレクション SelListVw.ColumnHeaders の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
			.Columns.Item(5).TextAlign = System.Windows.Forms.HorizontalAlignment.Right
			'UPGRADE_WARNING: コレクション SelListVw.ColumnHeaders の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
			.Columns.Item(6).TextAlign = System.Windows.Forms.HorizontalAlignment.Center
			'UPGRADE_WARNING: コレクション SelListVw.ColumnHeaders の下限が 1 から 0 に変更されました。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A3B628A0-A810-4AE2-BFA2-9E7A29EB9AD0"' をクリックしてください。
			.Columns.Item(7).TextAlign = System.Windows.Forms.HorizontalAlignment.Center
		End With
		SelListVw.Width = VB6Conv.TwipsToPixelsX(12900 + 330) 'スクロールバー分プラス
		...
		'UPGRADE_ISSUE: Form プロパティ SentakMitumori_cls.ScaleWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
...
Form_Load_Err:
	...
	'UPGRADE_WARNING: イベント SentakMitumori_cls.Resize は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub SentakMitumori_cls_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub SentakMitumori_cls_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
...
EventExitSub:
	...
	Private Sub SentakMitumori_cls_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
...
Item_Check_Err:
	...
	Private Function Download() As Boolean
		...
		'    Dim cMitumoriH As clsMitumoriH
		...
		'製品クラス生成
		'    Set cMitumoriH = New clsMitumoriH

		rs = m_Parent.GetRs([tx_得意先CD].Text, [tx_納入先CD].Text, [tx_s見積番号].Text, [tx_e見積番号].Text, [tx_見積件名].Text, [tx_s見積金額].Text, [tx_e見積金額].Text, idc(0).Text, idc(1).Text)
		...
		Do Until rs.EOF
			...
			''        buf = String(7, " ")
			''        RSet buf = CStr(rs(1).Value)
			''        itmX.ListSubItems.Add , , CStr(buf)
			If NullToZero((rs.Fields(1).Value), "").ToString = "" Then
				itmX.SubItems.Add("")
			Else
				...
				itmX.SubItems.Add(CType(NullToZero((rs.Fields(1).Value), ""), DateTime).ToString("yy/MM/dd"))
			End If
			' ''        Select Case CInt(NullToZero(rs(2).Value, 0))
			' ''            Case 0
			' ''                ret = "新店"
			' ''            Case 1
			' ''                ret = "改装"
			...
			' ''                ret = "移転"
			' ''            Case 4
			' ''                ret = "補充"    '2003/12/04.ADD
			' ''        End Select
			' ''
			' ''        itmX.ListSubItems.Add , , ret
			itmX.SubItems.Add(ModKubuns.Get物件種別名(NullToZero((rs.Fields(2).Value), 0))) '2012/09/06 ADD
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CStr(NullToZero((rs.Fields(3).Value), "")))
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CStr(NullToZero((rs.Fields(4).Value), "")))
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CType(NullToZero((rs.Fields(5).Value), ""), Integer).ToString("#,##0"))
			If NullToZero((rs.Fields(6).Value), "").ToString = "" Then
				itmX.SubItems.Add("")
			Else
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				itmX.SubItems.Add(CType(NullToZero((rs.Fields(6).Value), ""), DateTime).ToString("yy/MM/dd"))
			End If
			If NullToZero((rs.Fields(7).Value), "").ToString = "" Then
				itmX.SubItems.Add("")
			Else
				'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				itmX.SubItems.Add(CType(NullToZero((rs.Fields(7).Value), ""), DateTime).ToString("yy/MM/dd"))
			End If
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CStr(NullToZero((rs.Fields(8).Value), "")))
			rs.MoveNext()
		Loop
		ReleaseRs(rs)
