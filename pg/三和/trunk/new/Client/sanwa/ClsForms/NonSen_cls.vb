...
Public Class NonSen_cls
	...
	Private Sub NonSen_cls_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
	...
	Private Sub NonSen_cls_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub NonSen_cls_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		'Dim i, j As Integer
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "納入先CD", CInt(VB6Conv.TwipsToPixelsX(1050)))
			.Columns.Add("", "納入先名1", CInt(VB6Conv.TwipsToPixelsX(3500)))
			.Columns.Add("", "納入先名2", CInt(VB6Conv.TwipsToPixelsX(3500)))
		End With
		...
		[tx_得意先CD].Enabled = False 'NOTE SS
		If Me.得意先CD = "" Then
			[rf_得意先名].Text = ""
		Else
			With cTokuisaki
...
Form_Load_Err:
	...
	Private Sub NonSen_cls_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
	...
	Private Function Download() As Boolean
		...
		If [tx_検索ID].Text <> vbNullString Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & " 納入先CD >= '" & SQLString([tx_検索ID].Text) & "'"
		End If
		If tx_検索名.Text <> vbNullString Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & " 納入先名1 LIKE '%" & SQLString(tx_検索名.Text) & "%'"
		End If
