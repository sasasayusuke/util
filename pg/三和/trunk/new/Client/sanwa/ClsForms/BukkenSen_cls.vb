...
Public Class BukkenSen_cls
	...
	Private Sub BukkenSen_cls_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "物件番号", CInt(VB6Conv.TwipsToPixelsX(1150)))
			.Columns.Add("登録日付", CInt(VB6Conv.TwipsToPixelsX(1150)), System.Windows.Forms.HorizontalAlignment.Center)
			.Columns.Add("", "得意先", CInt(VB6Conv.TwipsToPixelsX(1000)))
			.Columns.Add("", "納入先", CInt(VB6Conv.TwipsToPixelsX(1000)))
			.Columns.Add("", "物件名", CInt(VB6Conv.TwipsToPixelsX(4500)))
			.Columns.Add("更新日付", CInt(VB6Conv.TwipsToPixelsX(1700)), System.Windows.Forms.HorizontalAlignment.Center)
		End With
...
Form_Load_Err:
	...
	Private Sub BukkenSen_cls_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub BukkenSen_cls_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
...
EventExitSub:
	...
	Private Sub BukkenSen_cls_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
	...
	Private Sub tx_担当者CD__KeyDown(sender As Object, e As KeyEventArgs) Handles tx_担当者CD.KeyDown
		ReturnF = True
	End Sub
...
EventExitSub:
	...
	Private Sub tx_物件名_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_物件名.KeyDown
		ReturnF = True
	End Sub
	...
	Private Sub tx_s物件登録日Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_s物件登録日Y.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'If LenB([tx_s物件登録日Y].Text & Chr(KeyAscii)) - LenB([tx_s物件登録日Y].SelectedText) = [tx_s物件登録日Y].Maxlength Then

			' エンコーディングを選択
			Dim encoding As System.Text.Encoding = System.Text.Encoding.GetEncoding("Shift-JIS")

			' TextとSelectedTextのバイト数を計算
			Dim totalByteCount As Integer = encoding.GetByteCount([tx_s物件登録日Y].Text & ChrW(KeyAscii))
			Dim selectedByteCount As Integer = encoding.GetByteCount([tx_s物件登録日Y].SelText)

			' MaxLengthと比較してチェック
			If totalByteCount - selectedByteCount = [tx_s物件登録日Y].MaxLength Then
				ReturnF = True
				[tx_s物件登録日Y].Focus()
			End If
	...
	Private Sub tx_s物件登録日Y_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_s物件登録日Y.KeyDown
		ReturnF = True
	End Sub
	...
	Private Sub tx_s物件登録日M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_s物件登録日M.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			'If LenB([tx_s物件登録日M].Text & Chr(KeyAscii)) - LenB([tx_s物件登録日M].SelectedText) = [tx_s物件登録日M].MaxLength Then

			' 適切なエンコーディングを選択
			Dim encoding As System.Text.Encoding = System.Text.Encoding.GetEncoding("Shift-JIS")

			' TextとSelectedTextのバイト数を計算
			Dim totalByteCount As Integer = encoding.GetByteCount([tx_s物件登録日M].Text & ChrW(KeyAscii))
			Dim selectedByteCount As Integer = encoding.GetByteCount([tx_s物件登録日M].SelText)

			' MaxLengthと比較してチェック
			If totalByteCount - selectedByteCount = [tx_s物件登録日M].MaxLength Then
				If KeyAscii <> 0 Then
					ReturnF = True
					[tx_s物件登録日M].Focus()
				End If
	...
	Private Sub tx_s物件登録日M_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_s物件登録日M.KeyDown
		ReturnF = True
	End Sub
	...
	Private Sub tx_s物件登録日D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_s物件登録日D.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			'If LenB([tx_s物件登録日D].Text & Chr(KeyAscii)) - LenB([tx_s物件登録日D].SelectedText) = [tx_s物件登録日D].Maxlength Then

			' 適切なエンコーディングを選択
			Dim encoding As System.Text.Encoding = System.Text.Encoding.GetEncoding("Shift-JIS")

			' TextとSelectedTextのバイト数を計算
			Dim totalByteCount As Integer = encoding.GetByteCount([tx_s物件登録日D].Text & ChrW(KeyAscii))
			Dim selectedByteCount As Integer = encoding.GetByteCount([tx_s物件登録日D].SelText)

			' MaxLengthと比較してチェック
			If totalByteCount - selectedByteCount = [tx_s物件登録日D].MaxLength Then
	...
	Private Sub tx_s物件登録日D_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_s物件登録日D.KeyDown
		ReturnF = True
	End Sub
	...
	Private Sub tx_e物件登録日Y_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_e物件登録日Y.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			'If LenB([tx_e物件登録日Y].Text & Chr(KeyAscii)) - LenB([tx_e物件登録日Y].SelectedText) = [tx_e物件登録日Y].Maxlength Then
	...
	Private Sub tx_e物件登録日Y_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_e物件登録日Y.KeyDown
		ReturnF = True
	End Sub
	...
	Private Sub tx_e物件登録日M_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_e物件登録日M.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			'If LenB([tx_e物件登録日M].Text & Chr(KeyAscii)) - LenB([tx_e物件登録日M].SelectedText) = [tx_e物件登録日M].Maxlength Then
	...
	Private Sub tx_e物件登録日M_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_e物件登録日M.KeyDown
		ReturnF = True
	End Sub
	...
	Private Sub tx_e物件登録日D_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles tx_e物件登録日D.KeyPress
		...
		If KeyAscii <> 8 Then ' バックスペースは例外
			...
			'UPGRADE_ISSUE: 定数 vbFromUnicode はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="55B59875-9A95-4B71-9D6A-7C294BF7139D"' をクリックしてください。
			'UPGRADE_ISSUE: LenB 関数はサポートされません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"' をクリックしてください。
			'If LenB([tx_e物件登録日D].Text & Chr(KeyAscii)) - LenB([tx_e物件登録日D].SelectedText) = [tx_e物件登録日D].Maxlength Then
	...
	Private Sub tx_e物件登録日D_KeyDown(sender As Object, e As KeyEventArgs) Handles tx_e物件登録日D.KeyDown
		ReturnF = True
	End Sub
...
Item_Check_Err:
	...
	Private Function Download() As Boolean
		...
		Do Until rs.EOF
			...
			itmX.SubItems.Add(CType(NullToZero((rs.Fields("物件登録日付").Value), ""), DateTime).ToString("yy/MM/dd")) '2007/02/16 ADD
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			...
			'UPGRADE_WARNING: オブジェクト NullToZero() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			itmX.SubItems.Add(CType(NullToZero((rs.Fields("登録変更日").Value), ""), DateTime).ToString("yy/MM/dd hh:mm")) '2012/10/26 ADD
			rs.MoveNext()
		Loop
		ReleaseRs(rs)
