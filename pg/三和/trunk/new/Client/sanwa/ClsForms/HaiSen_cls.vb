...
Public Class HaiSen_cls
	...
	Private Sub HaiSen_cls_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
		Dim Cancel As Boolean = e.Cancel
		Dim UnloadMode As System.Windows.Forms.CloseReason = e.CloseReason
		'    Set ResultCodeSetControl = Nothing
		'UPGRADE_NOTE: オブジェクト HaiSen_cls をガベージ コレクトするまでこのオブジェクトを破棄することはできません。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"' をクリックしてください。
		'Me = Nothing
		Me.Dispose()
		e.Cancel = Cancel
	End Sub

	'UPGRADE_WARNING: イベント HaiSen_cls.Resize は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub HaiSen_cls_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub HaiSen_cls_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		'Dim i, j As Integer
		...
		With SelListVw
			.Columns.Clear()
			.Columns.Add("", "配送先CD", CInt(VB6Conv.TwipsToPixelsX(1050)))
			.Columns.Add("", "配送先名1", CInt(VB6Conv.TwipsToPixelsX(3500)))
			.Columns.Add("", "配送先名2", CInt(VB6Conv.TwipsToPixelsX(3500)))
		End With
		...
		'UPGRADE_ISSUE: Form プロパティ HaiSen_cls.ScaleWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
...
Form_Load_Err:
	...
	Private Sub HaiSen_cls_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
...
Item_Check_Err:
	...
	Private Function Download() As Boolean
		...
		sql = "SELECT 配送先CD, 配送先名1, 配送先名2 " & "FROM TM配送先 "
		...
		If [tx_検索ID].Text <> vbNullString Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & " 配送先CD >= '" & SQLString([tx_検索ID].Text) & "'"
		End If
		If tx_検索名.Text <> vbNullString Then
			If whr <> "" Then
				whr = whr & " and "
			End If
			whr = whr & " 配送先名1 LIKE '%" & SQLString(tx_検索名.Text) & "%'"
		End If
		...
		Do Until rs.EOF
			'UPGRADE_WARNING: TextBox プロパティ tx_検索ID.MaxLength には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
			Buf = New String("0"c, [tx_検索ID].MaxLength) & "0" 'ひとつ余計にする
			If ISInt((rs.Fields(0).Value)) Then
				Buf = RSet(CStr(rs.Fields(0).Value), Len(rs.Fields(0).Value))
			Else
