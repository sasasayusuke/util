...
Public Class SentakNM_cls
	...
	Private Sub SentakNM_cls_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
	...
	Private Sub SentakNM_cls_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Resize
	...
	Private Sub SentakNM_cls_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Me.Load
		Dim i, j As Integer
		...
		For i = 0 To FieldCnt
			...
			If IsDBNull(List_ColWidth(i)) Then
				If NameLen(i) > FieldSize(i) Then
					...
					List_ColWidth(i) = VB6Conv.TextWidth(New String(" ", NameLen(i)), SelListVw.Font) + 200
				Else
					'UPGRADE_ISSUE: Form メソッド SentakNM_cls.TextWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
					'UPGRADE_WARNING: オブジェクト List_ColWidth(i) の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
					List_ColWidth(i) = VB6Conv.TextWidth(New String(" ", FieldSize(i)), SelListVw.Font) + 200
				End If
			End If
		Next
		ReleaseRs(rs)
		...
		With SelListVw
			.Columns.Clear()
			For i = 0 To FieldCnt
				.Columns.Add("", FieldName(i), CInt(VB6Conv.TwipsToPixelsX(List_ColWidth(i))))
			Next
		End With
		...
		For i = 0 To FieldCnt
			...
			SelListVw.Width = VB6Conv.TwipsToPixelsX(VB6Conv.PixelsToTwipsX(SelListVw.Width) + List_ColWidth(i))
		Next
		SelListVw.Width = VB6Conv.TwipsToPixelsX(VB6Conv.PixelsToTwipsX(SelListVw.Width) + 330) 'スクロールバー分プラス
...
Form_Load_Err:
	...
	Private Sub SentakNM_cls_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
...
EventExitSub:
	...
	Private Sub SetupBlank()
		...
		'2015/06/11 ADD↓
		'UPGRADE_ISSUE: Form メソッド SentakNM_cls.TextWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
		If lb_検索ID.Width < VB6Conv.TextWidth(lb_検索ID.Text, lb_検索ID.Font) Then
			'UPGRADE_ISSUE: Form メソッド SentakNM_cls.TextWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
			lb_検索ID.Width = VB6Conv.TextWidth(lb_検索ID.Text, lb_検索ID.Font) + 20
			[tx_検索ID].Left = lb_検索ID.Width + lb_検索ID.Left
		End If
		'2015/06/11 ADD↑
		...
		'2015/06/11 ADD↓
		'UPGRADE_ISSUE: Form メソッド SentakNM_cls.TextWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
		If lb_検索名.Width < VB6Conv.TextWidth(lb_検索名.Text, lb_検索名.Font) Then
			'UPGRADE_ISSUE: Form メソッド SentakNM_cls.TextWidth はアップグレードされませんでした。 詳細については、'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"' をクリックしてください。
			'lb_検索名.Width = VB6Conv.TextWidth(lb_検索名.Text, lb_検索名.Font) + 20
			lb_検索名.Width = lb_検索ID.Width
			[tx_検索名].Left = lb_検索名.Width + lb_検索名.Left
		End If
		'2015/06/11 ADD↑
...
Item_Check_Err:
	...
	Private Function Download() As Boolean
		...
		If IsCheckNull([tx_検索ID].Text) = False Or IsCheckNull([tx_検索名].Text) = False Then
			If IsCheckNull([tx_検索ID].Text) = False Then
				whr = pFind_ID & " >= " & [tx_検索ID].Text
				If IsCheckNull([tx_検索名]) = False Then
					whr = whr & " AND " & pFind_Name & " LIKE '%" & SQLString([tx_検索名].Text) & "%'"
				End If
			Else
				If IsCheckNull([tx_検索名]) = False Then
					If pFindBase_Name = vbNullString Then
						whr = pFind_Name & " LIKE '%" & SQLString([tx_検索名].Text) & "%'"
					Else
						whr = pFindBase_Name & " LIKE '%" & SQLString([tx_検索名].Text) & "%'"
					End If
				End If
			End If
		End If
	...
	Public Sub SelSetup(ByRef sql As String, ByRef FindID As String, ByRef FindNM As String, ByRef FindBaseNM As String, ByRef Pkey As String, ByRef Title As String, ParamArray ByVal FieldsWidth() As Object)
