...
''' </summary>
Friend Class clsWelBukkenKubun

	'変数
	Private m_ウエルシア物件区分CD As String
	Private m_ウエルシア物件区分名 As String
	...
	'   ウエルシア物件区分CD
	'//////////////////////////////////////
	Public Property ウエルシア物件区分CD() As String
		Get
			[ウエルシア物件区分CD] = m_ウエルシア物件区分CD
		End Get
	...
	'   ウエルシア物件区分名
	'//////////////////////////////////////
	Public Property ウエルシア物件区分名() As String
		Get
			[ウエルシア物件区分名] = m_ウエルシア物件区分名
		End Get
		Set(ByVal Value As String)
			m_ウエルシア物件区分名 = Value
		End Set
	...
	Public Sub Initialize()
		'初期化
		'初期値を1桁多く設定する
		m_ウエルシア物件区分CD = New String(" "c, m_MaxLength + 1)
		m_ウエルシア物件区分名 = vbNullString
	...
	Public Function GetbyID() As Boolean
		...
		sql = sql & " WHERE ウエルシア物件区分CD = " & SpcToNull((Me.ウエルシア物件区分CD), 0)
		...
		With rs
			...
			Else
				...
				Me.ウエルシア物件区分CD = .Fields("ウエルシア物件区分CD").Value
				Me.ウエルシア物件区分名 = .Fields("ウエルシア物件区分名").Value
...
GetbyID_Err:
	...
	Public Function GetAllRs() As ADODB.Recordset
		...
		'SQL生成
		sql = "SELECT TE.ウエルシア物件区分CD, TE.ウエルシア物件区分名, TE.初期登録日, TE.登録変更日"
		sql = sql & " FROM TMウエルシア物件区分 AS TE"
...
GetRs_Err:
	...
	Public Function GetRsByID(ByVal 開始CD As Object, ByVal 終了cd As Object) As ADODB.Recordset
		...
		'-------抽出条件セット
		whr = SQLIntRange("TE.ウエルシア物件区分CD", 開始CD, 終了cd)
...
GetRs_Err:
	...
	Public Function ShowDialog() As Boolean
		...
		With fSentak
			Call .SelSetup("SELECT ウエルシア物件区分CD,ウエルシア物件区分名 FROM TMウエルシア物件区分", "ウエルシア物件区分CD", "ウエルシア物件区分名", "", "ウエルシア物件区分CD", "ウエルシア物件区分選択", 1050, 5000)
			.tx_検索名.IMEMode = ExText.ExTextBox.IMEModeType.全角ひらがな
			.ShowDialog()
			If .DialogResult_Renamed Then
				Me.ウエルシア物件区分CD = CStr(.DialogResultCode)
				ShowDialog = True
			End If
		End With
	...
	Public Function Upload() As Boolean
		...
		sql = "SELECT  ウエルシア物件区分CD, ウエルシア物件区分名"
		sql = sql & " ,初期登録日, 登録変更日"
		sql = sql & " FROM TMウエルシア物件区分"
		sql = sql & " WHERE ウエルシア物件区分CD = " & Me.ウエルシア物件区分CD
		...
		With rs
			Select Case .EOF
				Case True
					.AddNew()
					.Fields("ウエルシア物件区分CD").Value = Me.ウエルシア物件区分CD

					.Fields("初期登録日").Value = Today
				Case False
			End Select

			.Fields("ウエルシア物件区分名").Value = Me.ウエルシア物件区分名
...
Trans_err: '---エラー時
	...
	Friend Function PurgeChk() As Boolean
		...
		cmd.CommandText = "usp_ChkDelForウエルシア物件区分"
		cmd.CommandType = ADODB.CommandTypeEnum.adCmdStoredProc

		' それぞれのパラメータの値を指定する
		With cmd.Parameters
			.Item(1).Value = Me.ウエルシア物件区分CD
		End With
