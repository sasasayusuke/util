...
''' 推進部クラス
''' </summary>
Friend Class clsSuishin

	'変数
	Private m_営業推進部CD As String
	...
	'   営業推進部CD
	'//////////////////////////////////////
	Public Property 営業推進部CD() As String
		Get
			[営業推進部CD] = m_営業推進部CD
		End Get
	...
	'   営業推進部名
	'//////////////////////////////////////
	Public Property 営業推進部名() As String
		Get
			[営業推進部名] = m_Name
		End Get
		Set(ByVal Value As String)
	...
	Public Sub Initialize()
		'初期化
		'初期値を1桁多く設定する
		m_営業推進部CD = New String(" "c, m_MaxLength + 1)
		m_Name = vbNullString
	...
	Public Function GetbyID() As Boolean
		...
		'SQL生成
		sql = "SELECT *"
		sql = sql & " FROM TM営業推進部"
		sql = sql & " WHERE 営業推進部CD = " & Me.営業推進部CD
		...
		With rs
			...
			Else
				...
				m_営業推進部CD = .Fields("営業推進部CD").Value
				m_Name = .Fields("営業推進部名").Value
			End If
		End With
...
GetRs_Err:
	...
	Public Function GetRsByID(ByVal 開始CD As Object, ByVal 終了CD As Object) As ADODB.Recordset
		...
		'-------抽出条件セット
		whr = SQLIntRange("営業推進部CD", 開始CD, 終了CD)
...
GetRs_Err:
	...
	Public Function ShowDialog() As Boolean
		...
		With fSentak
			Call .SelSetup("SELECT 営業推進部CD,営業推進部名 FROM TM営業推進部", "営業推進部CD", "営業推進部名", "", "営業推進部CD", "営業推進部選択", 1050, 2235)
			.ShowDialog()
			If .DialogResult_Renamed Then
				Me.営業推進部CD = CStr(.DialogResultCode)
				ShowDialog = True
			End If
		End With
	...
	Public Function Upload() As Boolean
		...
		With rs
			Select Case .eof
				Case True
					.AddNew()
					.Fields("営業推進部CD").Value = Me.営業推進部CD

					.Fields("初期登録日").Value = Today
				Case False
			End Select

			.Fields("営業推進部名").Value = Me.営業推進部名
...
Trans_err: '---エラー時
	...
	Friend Function PurgeChk() As Boolean
		...
		cmd.CommandText = "usp_ChkDelFor営業推進部"
		cmd.CommandType = ADODB.CommandTypeEnum.adCmdStoredProc

		' それぞれのパラメータの値を指定する
		With cmd.Parameters
			.Item(1).Value = Me.営業推進部CD
		End With
