<template>
  <v-app class="ssap">
    ...
    <!-- Content start -->
    <v-main class="ssap__main" :class="{ 'is-login': login }">
      <!-- Components loading -->
      ...
      <v-container id="component-loaded" fluid pt-0 px-0>
        <slot>
          <nuxt></nuxt>
        </slot>
      </v-container>
...
import { errorBarStore, fullScreenLoadingStore } from '~/store'
...
export default BaseVueClass.extend({
  ...
  computed: {
    login() {
      const path = this.$route.path as any
      return path === '/'
    },
  },
  props: {
...
.ssap__main {
  ...
  color: $c-black;
  &.is-login {
    background-image: url(@/assets/img/bo_back.png);
