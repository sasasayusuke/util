<template>
  <DetailContainer
    :title="$t('survey.pages.index.section.serviceAndcompletionTotal')"
    :is-editing="false"
  ...
  >
    ...
    <Sheet class="px-7 pt-1 pb-7">
      <SurveyTopServiceAndCompletionSort
        :param="searchParam"
        @update="update"
        @sort="search"
        @clear="clear"
      ...
      <v-container fluid pa-0>
        <v-row no-gutters>
          <v-col>
            <SurveyTopServiceAndCompletionTable
              v-if="!isLoading"
              :surveys="surveySummaryAll.termSummaryResult"
              :set-type="type"
            />
            <p class="mt-5 mb-0">
              <TextLink to="/survey/list?type=service_and_completion">
          ...
          <v-col class="pl-10">
            <Chart
              type="line"
              chart-id="SurveyTopServiceAndCompletionContainer-chart-1"
              :width="530"
              :height="180"
...
import SurveyTopServiceAndCompletionSort, {
  SearchParam,
} from '~/components/survey/organisms/SurveyTopServiceAndCompletionSort.vue'
import SurveyTopServiceAndCompletionTable from '~/components/survey/molecules/SurveyTopServiceAndCompletionTable.vue'
...
export default BaseComponent.extend({
  ...
  methods: {
    ...
    search() {
      ...
      this.$emit(this.apiName, 'ServiceAndCompletion', request)
    },
    /**
        ...
        if (
          ...
          for (const i2 in this.surveySummaryAll.surveys) {
            ...
            if (elm.summaryMonth === label) {
              const service = elm.service.satisfactionSummary
                ? elm.service.satisfactionSummary
                : 0
              const completion = elm.completion.satisfactionSummary
        ...
        datasets: [
          {
            ...
            data: this.getServiceAndCompletionData(),
            borderColor: '#ff061b',
