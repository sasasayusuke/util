...
class TestGetKarteById:
    ...
    def test_access_ok(
        ...
        response = client.get(f"/api/karten/{karte_id}", headers=REQUEST_HEADERS)
    ...
    def test_ok(self, mocker, mock_auth_admin, karte_id, model, expected):
        """正常系のテスト"""
    ...
    def test_karte_not_found(self, mocker, mock_auth_admin):
        mock_auth_admin([UserRoleType.SYSTEM_ADMIN.key])
        """カルテが存在しない時"""
    ...
    def test_auth_system_admin_ok(
        self, mocker, mock_auth_admin, karte_id, model, expected
    ):
        """権限のテスト：システム管理者：正常（出席支援者・出席お客様がなしの場合）"""
        mock_auth_admin([UserRoleType.SYSTEM_ADMIN.key])
    ...
    def test_auth_man_hour_ops_403(
        self,
        mocker,
        mock_auth_admin,
        karte_id,
    ...
    def test_auth_supporter_mgr_ok(self, mocker, mock_auth_admin, project_model):
        ...
        mock = mocker.patch.object(ProjectKarteModel, "get")
        mock.return_value = ProjectKarteModel(
            karte_id="3336e959-c216-46a8-92c3-3d7d3f951d00",
            project_id="2854fc20-caea-44bb-ada5",
            date="2022/08/31",
            start_datetime="2022/08/31 13:00",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [],
                # 非担当の非公開案件(メールアドレスが一致する一般ユーザ情報が存在しない場合)
                ProjectModel(
                    id="2854fc20-caea-44bb-ada5",
                    data_type=DataType.PROJECT,
                    salesforce_customer_id="c9b67094-cdab-494c-818e-d4845088269b",
