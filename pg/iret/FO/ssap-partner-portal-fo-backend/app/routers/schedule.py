...
def create_schedules(
    ...
    Args:
        body (CreateSchedulesRequest): 登録内容
...
def put_support_schedules_by_id_date(
    ...
    query_params: PutSupportSchedulesByIdDateQuery = Depends(),
    current_user: UserModel = Depends(
        get_current_user_factory(
            accessible_roles={
                UserRoleType.SUPPORTER.key,
    ...
    Args:
        ...
        PutSupportScheduleIdDateResponse(karte_id, version): クエリパラメータで指定
        current_user (Behavior, optional): 認証済みのユーザー

    Returns:
        PutSupportScheduleIdDateResponse: 更新結果
    """
    return SchedulesService.update_support_schedules_by_id_qnd_date(
        item=body,
...
def delete_support_schedules_by_id_date(
    ...
    Args:
        DeleteSupportSchedulesByIdDateQuery(karte_id, version): クエリパラメータで指定
        current_user (Behavior, optional): 認証済みのユーザー
