...
## 📋 概要
ソルバーソーシング機能（人材要件機能）は、法人ソルバーおよび個人ソルバーの登録・管理を行うシステムです。
法人ソルバーの情報管理から、所属する個人ソルバーの申請・承認・管理まで一貫したワークフローを提供します。

### 対象ユーザー
- APT（管理者）: 全機能利用可能
- SOLVER_STAFF（法人ソルバー）: 所属法人の機能のみ利用可能

### 業務上の目的
- 法人ソルバーの情報管理
- 個人ソルバーの申請・審査プロセス
- ソルバー候補から個人ソルバーへの登録フロー
- 稼働率・単価の定期更新
...
flowchart TD
    Start([開始]) --> Login[ログイン]
    Login --> Menu[ソルバーメニュー画面]
    Menu --> SelectCorp[法人ソルバー選択]
    SelectCorp --> CorpCheck{法人ソルバー情報<br/>登録済み？}
    
    CorpCheck -->|No| InitCorpInfo[法人ソルバー情報入力]
    InitCorpInfo --> CorpInfoSave[法人ソルバー情報保存]
    CorpInfoSave --> MenuChoice
    
    CorpCheck -->|Yes| MenuChoice{機能選択}
    
    MenuChoice -->|法人ソルバー情報詳細| CorpDetail[法人ソルバー情報詳細]
    MenuChoice -->|新規ソルバー申請| NewSolverApp[新規個人ソルバー申請]
    MenuChoice -->|ソルバー候補| CandidateList[ソルバー候補一覧]
    MenuChoice -->|ソルバー一覧| SolverList[個人ソルバー一覧]
    MenuChoice -->|稼働率・単価更新| UtilRateUpdate[稼働率・単価更新]
    
    %% 法人ソルバー情報詳細フロー
    CorpDetail --> CorpDetailView[法人ソルバー情報表示]
    CorpDetailView --> CorpDetailEdit[法人ソルバー情報変更]
    CorpDetailEdit --> CorpDetailSave[情報保存]
    CorpDetailSave --> Menu
    
    %% 新規個人ソルバー申請フロー
    NewSolverApp --> BasicInfo[個人ソルバー基本情報入力]
    BasicInfo --> ScreeningItems[スクリーニング項目入力]
    ScreeningItems --> CriteriaItems[クライテリア項目入力]
    CriteriaItems --> TempSave{一時保存？}
    TempSave -->|Yes| SaveTemp[一時保存実行]
    TempSave -->|No| ConfirmScreen[確認画面]
    SaveTemp --> SolverList
    ConfirmScreen --> SubmitApp[申請実行]
    SubmitApp --> AppComplete[申請完了]
    AppComplete --> SolverList
    
    %% ソルバー候補関連フロー
    CandidateList --> CandidateAction{アクション選択}
    CandidateAction -->|新規候補申請| NewCandidate[新規ソルバー候補申請]
    CandidateAction -->|候補詳細| CandidateDetail[ソルバー候補詳細]
    
    NewCandidate --> CandidateInfo[候補者情報入力]
    CandidateInfo --> CandidateSubmit[候補者申請実行]
    CandidateSubmit --> CandidateList
    
    CandidateDetail --> PromoteCheck{個人ソルバー登録？}
    PromoteCheck -->|Yes| PromoteSolver[個人ソルバーに登録]
    PromoteCheck -->|No| CandidateList
    PromoteSolver --> SolverList
    
    %% ソルバー一覧フロー
    SolverList --> SolverAction{アクション選択}
    SolverAction -->|詳細表示| SolverDetail[個人ソルバー詳細]
    SolverAction -->|編集| SolverEdit[個人ソルバー編集]
    SolverAction -->|削除| SolverDelete[個人ソルバー削除]
    
    SolverDetail --> SolverList
    SolverEdit --> SolverSave[変更保存]
    SolverSave --> SolverList
    SolverDelete --> SolverList
    
    %% 稼働率・単価更新フロー
    UtilRateUpdate --> RateInput[稼働率・単価入力]
    RateInput --> BulkUpdate[一括更新実行]
    BulkUpdate --> UpdateComplete[更新完了]
...
2. **条件分岐**
   - 法人ソルバー情報が未登録の場合: 初期設定モーダル表示
   - 法人ソルバー情報が登録済みの場合: メニューアイテム有効化

3. **初期設定（未登録時）**
   - モーダル表示: 「法人ソルバー情報を入力してください」
   - 必要な操作: 法人ソルバー情報の入力
   - 制限: 入力完了まで個人ソルバー登録不可

### STEP 2: 新規個人ソルバー申請
**実行画面**: 新規個人ソルバー申請画面
**トリガー**: 「新規ソルバー申請」ボタンクリック

1. **基本情報入力**
   - セクション: 個人ソルバー基本情報
   - 必須項目: 個人ソルバー名、個人ソルバーかな、性別、生年月日、職歴
   - 任意項目: 役職、連絡先、電話番号、画像、学歴、専門テーマ、実績等

2. **スクリーニング項目入力**
   - セクション: スクリーニング項目
   - 入力形式: 各項目について「はい/いいえ」の評価 + エビデンス（テキスト）
   - 項目数: 8項目

3. **クライテリア項目入力**
   - セクション: クライテリア項目
   - 入力形式: 各項目についてエビデンス（テキスト）
   - 項目数: 8項目
...
5. **内部処理**
   ...
   - ファイルアップロード: プロフィール画像、資料等
   - バリデーション: 必須項目、形式チェック

6. **結果**
   - 成功時: 「申請が完了しました」メッセージ表示
   - エラー時: 「個人ソルバー登録申請には、全必須項目の入力が必要です」
...
1. **候補者一覧表示**
   - 表示内容: 候補者の基本情報、登録ステータス
   - 操作: 詳細表示、編集、個人ソルバー登録

2. **個人ソルバー登録**
   - 確認モーダル: 「個人ソルバーへの登録」
   - 注意事項: 「登録後はソルバー候補一覧からは削除され、個人ソルバー一覧に表示されます」
   - 処理: 候補者から個人ソルバーへのステータス変更

3. **内部処理**
   - API呼び出し: `PATCH /solvers/{solver_id}`
   - ステータス更新: 候補者 → 個人ソルバー
...
1. **更新対象表示**
   - 表示形式: テーブル形式で個人ソルバー一覧
   - 更新項目: 稼働率（今月・来月）、単価（人月・時間）
