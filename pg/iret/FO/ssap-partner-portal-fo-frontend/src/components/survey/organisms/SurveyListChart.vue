  ...
  <Sheet class="pt-5 pb-12 px-8" elevation="3" color="#fff" rounded>
    <v-container pa-0>
      <v-row no-gutters>
        <v-col>
          <Title class="type5">
            {{ $t('survey.pages.admin.list.service.AverageSatisfactionMonth') }}
          </Title>
      ...
      <v-row no-gutters class="mt-10">
        <v-col>
          <Title class="type5">
            {{
              $t(
                'survey.pages.admin.list.service.AverageSatisfactionCumulative'
              )
            }}
          </Title>
          <Sheet style-set="outlined" class="mt-3 pa-3">
...
export class ChartOptionsScalesChildGrid {
  display: boolean = false
}

export class ChartOptionsScalesChildTitlePadding {
  top: number = 0
  left: number = 0
  right: number = 0
  bottom: number = 0
...
export class ChartOptionsScalesChildTitle {
  display: boolean = false
  text: string = ''
  padding: ChartOptionsScalesChildTitlePadding =
    new ChartOptionsScalesChildTitlePadding()
}

export class ChartOptionsScalesChildTicks {
  min: number = 0
  stepSize: number = 0
}

export class ChartOptionsScalesChildTicksWithMax {
  max: number = 0
  min: number = 0
  stepSize: number = 0
}

export class ChartOptionsScalesChildA {
  title: ChartOptionsScalesChildTitle = new ChartOptionsScalesChildTitle()
  ticks: ChartOptionsScalesChildTicks = new ChartOptionsScalesChildTicks()
  position: string = ''
  suggestedMin: number = 0
  suggestedMax: number = 0
}

export class ChartOptions1ScalesChildB {
  title: ChartOptionsScalesChildTitle = new ChartOptionsScalesChildTitle()
  ticks: ChartOptionsScalesChildTicksWithMax =
    new ChartOptionsScalesChildTicksWithMax()

  position: string = ''
  suggestedMin: number = 0
  suggestedMax: number = 0
  grid: ChartOptionsScalesChildGrid = new ChartOptionsScalesChildGrid()
}

export class ChartOptions2ScalesChildB {
  title: ChartOptionsScalesChildTitle = new ChartOptionsScalesChildTitle()
  ticks: ChartOptionsScalesChildTicks = new ChartOptionsScalesChildTicks()
  position: string = ''
  suggestedMin: number = 0
...
export class ChartOptionsScalesChildX {
  grid: ChartOptionsScalesChildGrid = new ChartOptionsScalesChildGrid()
}

export class ChartOptions1Scales {
  A: ChartOptionsScalesChildA = new ChartOptionsScalesChildA()
  B: ChartOptions1ScalesChildB = new ChartOptions1ScalesChildB()
  x: ChartOptionsScalesChildX = new ChartOptionsScalesChildX()
}

export class ChartOptions2Scales {
  A: ChartOptionsScalesChildA = new ChartOptionsScalesChildA()
  B: ChartOptions2ScalesChildB = new ChartOptions2ScalesChildB()
  x: ChartOptionsScalesChildX = new ChartOptionsScalesChildX()
}

export class ChartOptionsPluginsLegendLabels {
  boxWidth: number = 0
  boxHeight: number = 0
}

export class ChartOptionsPluginsLegend {
  labels: ChartOptionsPluginsLegendLabels =
    new ChartOptionsPluginsLegendLabels()
}

export class ChartOptionsPluginsDatalabels {
  display: boolean = false
}

export class ChartOptionsPlugins {
  legend: ChartOptionsPluginsLegend = new ChartOptionsPluginsLegend()
  datalabels: ChartOptionsPluginsDatalabels =
    new ChartOptionsPluginsDatalabels()
}

export class ChartOptions1 {
  responsive: boolean = false
  plugins: ChartOptionsPlugins = new ChartOptionsPlugins()
  scales: ChartOptions1Scales = new ChartOptions1Scales()
}

export class ChartOptions2 {
  responsive: boolean = false
  plugins: ChartOptionsPlugins = new ChartOptionsPlugins()
  scales: ChartOptions2Scales = new ChartOptions2Scales()
}
...
export default BaseComponent.extend({
  ...
  computed: {
    ...
    chartData1(): ChartData {
      ...
      if (this.type !== 'quick') {
        const satisfactionAverageDataset = this.getDataSet(
          this.$t('survey.pages.admin.list.service.AverageSatisfactionMonth'),
          'satisfactionAverage',
          labels,
          summaryMonthly
        )
      ...
      const receivesDataset = this.getDataSet(
        this.$t('survey.pages.admin.list.service.receiveMonth'),
        'receive',
        labels,
        summaryMonthly
      )
    ...
    chartData2(): ChartData {
      ...
      const receivesDataset = this.getDataSet(
        this.$t('survey.pages.admin.list.service.receiveCumulative'),
        'receive',
        labels,
        summaryAccumulation
      )
        ...
        scales: {
          A: {
            title: {
              ...
              text: String(
                this.$t(
                  'survey.pages.admin.list.service.AverageSatisfactionMonth'
                )
              ),
  ...
  methods: {
    /**
     * ラベル生成
     * @param summaryMonthly 要約月次
     */
    getLabels(summaryMonthly: SurveyListSummaryItem[]): string[] {
  ...
  props: {
    type: {
      type: String,
      default: 'service_and_completion',
    },
    /** 案件アンケート */
    surveysResponse: {
      type: Object,
