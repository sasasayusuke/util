...
class PutSupportScheduleIdDateQuery(CustomBaseModel):
    """支援スケジュール更新クエリクラス"""
...
class ProjectSurveyScheduleInfo(CustomBaseModel):
    ...
    is_resend_anonymous_survey: bool = Field(..., title="匿名アンケート再送可能か")
    version: int = Field(None, title="ロックキー（楽観ロック制御）", example=1)
...
class DeleteSurveySchedulesByIdDateQuery(CustomBaseModel):
    ...
    survey_id: str = Query(..., title="アンケートID")
    version: int = Query(..., title="ロックキー（楽観ロック制御）", example=1, ge=1)
...
class AddInfoForUpdateMultipleSurveySchedules(CustomBaseModel):
    """アンケートスケジュール登録情報クラス"""
...
class UpdateInfoForUpdateMultipleSurveySchedules(CustomBaseModel):
    """アンケートスケジュール更新情報クラス"""
...
class UpdateMultipleSurveySchedulesRequest(CustomBaseModel):
    """アンケートスケジュール複数登録・更新リクエストクラス"""
    ...
    add: List[AddInfoForUpdateMultipleSurveySchedules] = Field(..., title="追加")
    update: List[UpdateInfoForUpdateMultipleSurveySchedules] = Field(..., title="更新")
...
class DeleteMultipleSurveySchedulesRequest(CustomBaseModel):
    """アンケートスケジュール複数削除リクエストクラス"""
...
class BulkcheduleInfo(CustomBaseModel):
...
class BulkCreateSurveyScheduleRequest(CustomBaseModel):
    ...
    service: BulkcheduleInfo = Field(..., title="サービスアンケート")
    completion: BulkcheduleInfo = Field(..., title="修了アンケート")
