import {
  ...
  IBulkDateInfo,
  UpdateMultipleSurveySchedulesRequestAdd,
  UpdateMultipleSurveySchedulesRequestUpdate,
} from '@/types/Schedule'
import { OkResponse } from '~/models/Common'
...
export async function ResendSurveyById(id: string) {
  return await $api.get(`/surveys/resend/${id}`)
}

export class UpdateMultipleSurveySchedulesRequest {
  sendDate: string // 送信日(回答期限日から○営業日前:-30～-1, 特定日:2022/04/01)
  surveyLimitDate: Number // 回答期限日(99:月末最終営業日、1～30: ○営業日後, 0: なし, 101～130: 翌月月初○営業日(営業日に+100した数値))
  add: UpdateMultipleSurveySchedulesRequestAdd[]
  update: UpdateMultipleSurveySchedulesRequestUpdate[]
  ...
  constructor(
    ...
    this.surveyLimitDate = surveyLimitDate
    this.add = []
    add.forEach((id) =>
      this.add.push(new UpdateMultipleSurveySchedulesRequestAdd(id))
    )
    this.update = []
    update.forEach((id) =>
      this.update.push(new UpdateMultipleSurveySchedulesRequestUpdate(id))
    )
  }
}

export async function UpdateMultipleSurveySchedules(
  request: UpdateMultipleSurveySchedulesRequest[]
) {
  for await (const req of request) {
    await $api.put<OkResponse>(`/schedules/survey/multiple`, req)
  }
}

export class DeleteMultipleSurveySchedulesRequest {
  surveyIds!: string[]

  constructor(surveyIds: string[]) {
    this.surveyIds = surveyIds
  }
}

//DeleteMultipleSurveySchedules実行関数
export async function DeleteMultipleSurveySchedules(
  request: DeleteMultipleSurveySchedulesRequest[]
) {
  for await (const req of request) {
    await $api.put<OkResponse>(`/schedules/survey/multiple/delete`, req)
  }
}
