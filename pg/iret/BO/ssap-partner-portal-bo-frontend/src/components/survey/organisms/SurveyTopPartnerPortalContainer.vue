<template>
  <DetailContainer
    :title="$t('survey.pages.index.section.partnerPortal')"
    :is-editing="false"
    is-hide-footer
    is-hide-button1
    is-hide-button2
      ...
      <v-container fluid pa-0>
        <v-row>
          ...
          <v-col class="pl-10">
            <Chart
              type="line"
              chart-id="SurveyTopPartnerPortalContainer-chart-1"
              :width="530"
              :height="180"
...
import SurveyTopPartnerPortalSort, {
  SearchParam,
} from '~/components/survey/organisms/SurveyTopPartnerPortalSort.vue'
import SurveyTopPartnerPortalTable from '~/components/survey/molecules/SurveyTopPartnerPortalTable.vue'
...
export default BaseComponent.extend({
  ...
  data() {
    return {
      type: 'survey',
      labels: {
        survey: this.$t('survey.pages.index.table.label.surveyFunction'),
        manHour: this.$t('survey.pages.index.table.label.manHourFunction'),
        karte: this.$t('survey.pages.index.table.label.karteFunction'),
        masterKarte: this.$t(
          'survey.pages.index.table.label.masterKarteFunction'
        ),
      } as any,
  ...
  methods: {
    ...
    search() {
      ...
      this.$emit(this.apiName, 'PartnerPortal', request)
    },
    /**
        ...
        if (
          ...
          for (const i2 in this.surveySummaryAll.surveys) {
            ...
            if (elm.summaryMonth === label) {
              if (this.type === 'survey') {
                results[i] = elm.pp.surveySatisfactionAverage
                  ? elm.pp.surveySatisfactionAverage
                  : 0
              } else if (this.type === 'manHour') {
                results[i] = elm.pp.manHourSatisfactionAverage
                  ? elm.pp.manHourSatisfactionAverage
                  : 0
              } else if (this.type === 'karte') {
                results[i] = elm.pp.karteSatisfactionAverage
                  ? elm.pp.karteSatisfactionAverage
                  : 0
              } else if (this.type === 'masterKarte') {
                results[i] = elm.pp.masterKarteSatisfactionAverage
                  ? elm.pp.masterKarteSatisfactionAverage
                  : 0
              }
              break
            }
    ...
    chartData() {
      return {
        ...
        datasets: [
          {
            type: 'line',
            label: this.$t('survey.pages.index.table.label.partnerPortal'),
            data: this.getPartnerPortalData(),
            borderColor: '#ff061b',
