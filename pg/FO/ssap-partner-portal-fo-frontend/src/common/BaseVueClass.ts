...
import { errorBarStore, meStore } from '~/store'
import {
  UnauthorizedError,
  ForbiddenError,
  InternalServerError,
...
import { DEFAULT_BIRTHDAY, UNKNOWN } from '~/const'
...
export default Vue.extend({
  ...
  methods: {
    formatDate(date: Date, format: string = 'Y/M/D hh:mm') {
    ...
    getCurrentDate,
    adjustEmail,
    getDefaultBirthday(birthDay: string) {
      return birthDay === DEFAULT_BIRTHDAY || birthDay === ''
        ? UNKNOWN
        : birthDay
    },
    getEditableBirthday(birthDay: string) {
      return birthDay === DEFAULT_BIRTHDAY || birthDay === UNKNOWN
        ? ''
        : birthDay
    },
    getDefaultPrice(price: number) {
      return price ? Number(price).toLocaleString() : UNKNOWN
    },
    getPriceUnit(price: number) {
      return price ? this.$t('solver.row.pricePerPersonMonth.unit') : ''
    },
  },
})
