        ...
        <template v-for="(file, index) in links">
          <li :key="index" class="m-karter-aside-links__item">
            ...
            <Icon
              v-if="isNotCustomer"
              size="18"
              @click="clearFilesOnScreen(index)"
              >mdi-close</Icon
            >
          </li>
        </template>
...
import { Title, Icon, FileInput } from '~/components/common/atoms/index'
...
export default BaseComponent.extend({
  ...
  computed: {
    /**
     * ログインユーザーのロールがお客様の場合はファイルを削除不可にする
     * @returns {boolean} ファイル削除アイコンの表示非表示
     */
    isNotCustomer() {
      if (meStore.role !== ENUM_USER_ROLE.CUSTOMER) return true
    },
  },
  methods: {
    /**
