...
export default BaseComponent.extend({
  name: 'ProjectEditSupporterModal',
  components: {
    ModalEdit,
    Sheet,
    Button,
  ...
  methods: {
    ...
    async setUser() {
      if (this.selectedUserId) {
        ...
        this.localUser.customerName = this.selectedUserInfo.company
        if (this.selectedUserInfo.role.match(/supporter || business_mgr/)) {
        ...
        this.localUser.email = this.selectedUserInfo.email
      }
    },
    async suggestUsers(
    ...
    duplicationUserCheck(selectedId: string): string | boolean {
      ...
      // 元々入力されていたユーザー以外かつ入力値が既に同画面上で登録されている
      if (selectedId !== this.selectedUserId && ids.includes(selectedId)) {
        return this.$t(
      ...
      if (email) {
        ...
        await GetUsers(request).then((res) => {
          ...
          for (const i in supportersInfo) {
            ...
            this.localUser.customerName = supporterInfo.company
