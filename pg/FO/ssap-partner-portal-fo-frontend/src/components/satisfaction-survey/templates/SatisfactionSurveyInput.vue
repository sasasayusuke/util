<template>
  <RootTemplate class="t-SurveyInput" pb-10 pt-8 mb-10>
    <SurveyInformation
      v-if="isFinished || survey.isFinished"
      :survey="survey"
      :type="type"
      :is-anonymous="true"
    ...
    />
    <SurveyInputAnswer
      v-if="!isFinished && isStarted && !survey.isFinished"
...
export default BaseComponent.extend({
  ...
  data(): DataType {
    return {
      currentQuestionFlowIndex: 0, // -1の時、未開始
      isFinished: false,
      answers: [],
      currentPage: 1,
      questions: [],
  ...
  computed: {
     ...
     * @returns アンケート設問
     */
    currentQuestion(): QuestionInfo | undefined {
     ...
     * @returns アンケート回答
     */
    currentOtherAnswer(): string | undefined {
      if (!this.survey.isFinished) {
        return this.answers[
          this.questionFlow[this.currentQuestionFlowIndex].questionIndex
        ].otherAnswer
  ...
  methods: {
    ...
    submit() {
      const token = ''
      const answers: any = []
      // 満足度評価アンケートの場合は支援者の開示OK
      const isDisclosure = true
      ...
      // 親コンポーネントで回答送信
      const answerData: UpdateSurveyOfSatisfactionByIdRequest = {
        token,
        answers,
        isFinished,
        isDisclosure,
