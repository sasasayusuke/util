...
export default BaseComponent.extend({
  ...
  computed: {
    ...
    isAnonymousWithFinished(): boolean {
      return this.isAnonymous && this.survey.isFinished
    },
    /**
        ...
        this.surveyMaster.questionFlow.forEach(
          (questionFlow: QuestionFlowInfo) => {
            ...
            if (questionFlow.conditionId === '') {
              ...
              if (
                questionFlow.conditionChoiceIds &&
                questionFlow.conditionChoiceIds.map
              ) {
                ...
                const isMatch = questionFlow.conditionChoiceIds.map(
                  (conditionChoiceId: string) => {
                    if (
                      this.survey.answers &&
                      this.survey.answers[answerIndex] &&
                      this.survey.answers[answerIndex].choiceIds
                    ) {
                      return this.survey.answers[
                        answerIndex
                      ].choiceIds.includes(conditionChoiceId)
                    } else {
                      return false
                    }
                  }
