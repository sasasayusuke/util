{
    // 見積書出力
    let dialogId = "EstimateOutput";
    let dialogName = "見積書出力";
    let category = "見積・発注";
    let btnLabel = "出力";
    ...
    [
        { type: 'button_inline', id: 'output', label: 'Excel出力', options: {
            icon:'disk',
            onclick:`estimateOutput_report('${dialogId}','${category}','${dialogName}','${btnLabel}', 'excel');`
        }},

        { type: 'button_inline', id: 'output', label: 'PDF出力', options: {
            icon:'disk',
            onclick:`estimateOutput_report('${dialogId}','${category}','${dialogName}','${btnLabel}', 'pdf');`
        }},
    ]);
    // ダイアログを開くボタンを追加
...
async function estimateOutput_report(dialogId, category, dialogName, btnLabel, format) {
    try {
        // バリデーション
        ...
        const estimate_no = $('#EstimateOutput_estimateNo');
        if(estimate_no.val() == ""){
            alert('見積番号が未入力です。');
            estimate_no.focus();
            return;
        ...
        let item_params = {
            "@i見積番号": SpcToNull($('#EstimateOutput_estimateNo').val()),
            "書式区分": SpcToNull($('#EstimateOutput_FormatCategoryFrom').val()),
            "表示担当": SpcToNull($('#EstimateOutput_displayManagerFrom').val()),
        ...
        let filename = "見積書" + "-" + estimate_no.val() +  SpcToNull($('#EstimateOutput_estimateName').val()) + dt + (format == "excel" ? ".xlsx":".pdf");
