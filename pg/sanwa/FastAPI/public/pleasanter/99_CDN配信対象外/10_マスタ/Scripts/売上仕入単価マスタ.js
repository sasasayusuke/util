...
$(document).on('input','#Results_ClassJ',async function(){
    try{
        ...
        $p.set($p.getControl('ClassJ'),arr[0].trim());
        $p.set($p.getControl('ClassG'),arr[1].trim());
        ...
        $p.set($p.getControl('ClassH'),cls_product["漢字名称"]);
        $p.set($p.getControl('NumI'),null_to_zero(cls_product["W"],0));
        $p.set($p.getControl('NumJ'),null_to_zero(cls_product["D"],0));
        $p.set($p.getControl('NumK'),null_to_zero(cls_product["H"],0));
        $p.set($p.getControl('NumL'),null_to_zero(cls_product["D1"],0));
