...
class tokuisakibetunenkanuriagezyunisuiihyoService(BaseService):
    """得意先別年間売上順位推移表サービス"""

    def display(self, request, session) -> Dict[str, Any]:

        # 支払予定表の印刷処理
        logger.info("【START】得意先別年間売上順位推移表処理")

        storedname = "usp_TK1301得意先別年間売上順位推移表_年度"
        ...
        wb = create_excel_object('Template得意先別年間売上順位推移表.xlsx')
        ...
        with get_excel_buffer() as buffer:
            ...
            if yearcount == 1:
                column_width = {'J': 0.64, 'K': 8.8, 'M': 8.8, 'N': 8.8, 'O': 8.8, 'P': 8.8}
                ws.delete_cols(10,6)
                ...
                for i in range(1,yearcount):
                    ...
                    column_width = {'4': 4.9, '5': 7.7, '7': 8, '8': 13.6, '9': 6.5}
                    for col, width in column_width.items():
                        # 行の幅を変更
            ...
            for row_data in storedresults['results'][0]:
                ...
                for year_num in range(1,yearcount+1):
                    ws.cell(row=3, column=4 + column_offset, value="{}年".format(int(request.params['@iS集計日付'][:4]) + year_num))._style = copy(ws.cell(row=3, column=4)._style)
                    ws.cell(row=row_num, column=4 + column_offset, value=row_data['順位{}'.format(year_num)])._style = copy(ws.cell(row=start_row, column=4 + column_offset)._style)
                    ...
                    ws.cell(row=row_num, column=6 + column_offset, value=row_data['原価金額{}'.format(year_num)])._style = copy(ws.cell(row=start_row, column=6 + column_offset)._style)
                    ...
                    if row_data['税抜金額{}'.format(year_num)] == 0:
                        ws.cell(row=row_num, column=7 + column_offset, value="")._style = copy(ws.cell(row=start_row, column=7 + column_offset)._style)
                    else:
                        ws.cell(row=row_num, column=7 + column_offset, value=row_data['原価金額{}'.format(year_num)] * 100 / row_data['税抜金額{}'.format(year_num)])._style = copy(ws.cell(row=start_row, column=7 + column_offset)._style)
                    ws.cell(row=4, column=7 + column_offset, value="=IF(ISERROR({column_num1}4*100/{column_num2}4),0,{column_num1}4*100/{column_num2}4)".format(column_num1=get_column_letter(6 + column_offset), column_num2=get_column_letter(8 + column_offset)))
                    ws.cell(row=row_num, column=8 + column_offset, value=row_data['税抜金額{}'.format(year_num)])._style = copy(ws.cell(row=start_row, column=8 + column_offset)._style)
            ...
            for year_num in range(1,yearcount+1):
                ...
                for row_data in storedresults['results'][0]:
                    # 原価率
                    rule1_1 = FormulaRule(formula = ["={}{}>=90".format(get_column_letter(7 + column_offset),row_num)], font = Font(color='FF0000'))
                    ws.conditional_formatting.add('{}{}'.format(get_column_letter(7 + column_offset),row_num), rule1_1)

                    # 順位
                    rule2 = FormulaRule(formula = ["={column}{row}>OFFSET({column}{row},0,-6)".format(column=get_column_letter(4 + column_offset),row=row_num)], font = Font(color='FF0000'))
                    ws.conditional_formatting.add('{}{}'.format(get_column_letter(4 + column_offset),row_num), rule2)

                    # 前対比
                    rule3 = FormulaRule(formula = ["=IF(ISERROR({column}{row}*100/OFFSET({column}{row},0,-6)),FALSE,{current_column}{row}<100)".format(column=get_column_letter(8 + column_offset),row=row_num,current_column=get_column_letter(5 + column_offset))], font = Font(color='FF0000'))
                    ws.conditional_formatting.add('{}{}'.format(get_column_letter(5 + column_offset), row_num), rule3)

                    # 純売上金額
                    ws.conditional_formatting.add('{}{}'.format(get_column_letter(8 + column_offset), row_num), rule3)
