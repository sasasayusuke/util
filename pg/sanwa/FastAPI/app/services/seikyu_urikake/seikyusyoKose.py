...
class SeikyusyoForKoseService(BaseService):
    """請求書発行 コーセー専用"""
    ...
    def display(self, request, session) -> Dict[str, Any]:
        ...
        # 請求書発行 コーセー専用の印刷処理
        logger.info("【START】請求書発行 コーセー専用")
        ...
        # 見積明細チェック
        if request.params["@i見積明細番号"] == "":
            raise ServiceError('見積明細番号が未入力です。')
        
        cMitumoriH.estimate_category = request.params["@i見積明細番号"]
        if not cMitumoriH.GetbyID(session):
            raise ServiceError('指定の見積明細番号は存在しません。')
        elif cMitumoriH.原価率 > 100:
        ...
        stored_results = self.download(session,request.params["@i見積番号"],request.params["@i見積明細番号"],request.params["@i請求書発行日付"])
        ...
        with get_excel_buffer() as buffer:
            ...
            # 何ページ目か計算
            page_count = math.ceil((row - 38) / 35)
            row = page_count * 35 + 38
    ...
    def download(self,session,estimate_no,details_no,billing_output_date):
        ...
        stored_name = "usp_SE0802請求書発行_客在_コーセー"
        outputparams = {"@RetST":"INT", "@RetMsg":"VARCHAR(255)"}
    ...
    def SeikyusyoMin(self,session,ws,copy_ws,GrsHead,GrsHeadU,GrsSheet,params):
        ...
        for record in GrsSheet:
            ...
            if siwake_no != record["仕分番号"]:
                ...
                ws.cell(row=row,column=3,value=record["仕分名称"])
