...
class UrikakekinsyukeihyoService(BaseService):
    """売掛金集計表サービス"""

    def display(self, request, session) -> StreamingResponse:

        # 売掛金集計表の印刷処理
        logger.info("【START】売掛金集計表処理")

        storedname="usp_HK0300売掛金集計表出力"
        storedname2="usp_売掛集計v2"
        ...
        if storedresults2["output_values"]["@RetST"] == -1:
            raise ServiceError(storedresults2["output_values"]["@RetMsg"])
        ...
        with get_excel_buffer() as buffer:
            wb = create_excel_object('Template_売掛金集計表.xlsx')
            ws = wb["Template"]
            ts = wb["Copy"]

            urikakeDate = request.params["@iDATE"]
            customerCodeFrom = request.params["@i開始得意先"]
            customerCodeCodeTo = request.params["@i終了得意先"]

            # ヘッダー設定
            ws["E1"] = str(urikakeDate)[:4] + '年' + str(urikakeDate)[5:7] + '月末締    '
            ws["E2"] = '[' + (customerCodeFrom if customerCodeFrom is not None else "最初") + ']'
            ws["I2"] = '[' + (customerCodeCodeTo if customerCodeCodeTo is not None else "最後") + ']'
            ws["BH1"] = datetime.now() 
            ...
            for result in storedresults['results'][0]:
                ...
                ws.cell(row=current_row+1, column=19, value=result['売上金額'])
                ws.cell(row=current_row+1, column=23, value=result['返品金額'])
                ws.cell(row=current_row+1, column=27, value=result['訂正金額'])
                ws.cell(row=current_row+1, column=31, value=result['差引売上額'])
                ws.cell(row=current_row+1, column=35, value=result['消費税額'])
                ws.cell(row=current_row+1, column=39, value=result['税込純売上額'])
                ws.cell(row=current_row+1, column=43, value=result['入金金額'])
    ...
    def display_siwake(self, request, session) -> StreamingResponse:
        ...
        # 売掛金集計表の印刷処理
        logger.info("【START】売掛金集計表仕分処理")

        storedname="usp_HK0301売掛金集計表仕訳出力"
