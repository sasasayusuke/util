...
class UrikakekinmototyoService(BaseService):
    """売掛金元帳サービス"""

    def display(self, request, session) -> StreamingResponse:

        # 売掛金元帳の印刷処理
        logger.info("【START】売掛金元帳出力処理")

        storedname1="usp_SE0400売掛金元帳H"
        storedname2="usp_SE1100売掛金元帳M"
        storedname3="usp_請求集計V2"

        # 請求集計処理
        params3 = {
            "@is得意先CD": request.params['@is得意先CD'],
            "@ie得意先CD": request.params['@ie得意先CD'],
            "@iSDate": request.params['@iS請求日付'],
            "@iEDate": request.params['@iE請求日付'],
        ...
        with get_excel_buffer() as buffer:
            wb = create_excel_object('Template_売掛金元帳.xlsx')
            ws = wb["Template"]
            ts = wb["Copy"]
            ...
            # 三和情報追加
            company_info = UrikakekinmototyoService.getKaisya(session, request)
            ...
            for result in storedresults1['results'][0]:
                ...
                ws['U{}'.format(current_row+9)] = result['入金金額']
                ws['U{}'.format(current_row+10)] = result['調整金額']
                ws['AA{}'.format(current_row+10)] = result['前月残高'] - result['入金金額'] - result['調整金額']
                ws['AG{}'.format(current_row+10)] = result['売上金額']
                ws['AM{}'.format(current_row+10)] = result['返品金額']
                ws['AS{}'.format(current_row+10)] = result['訂正金額']
                ws['AY{}'.format(current_row+10)] = result['消費税額']
                ws['BE{}'.format(current_row+10)] = result['売上金額'] + result['返品金額'] + result['訂正金額'] + result['消費税額']
                ...
                for result_detail in storedresults2['results'][0]:
                    ...
                    if result_detail['区分'] != 3:
                        if result_detail['見積行番号'] < 9000:
                            if result_detail['区分'] == 0:
                                ws.cell(row=current_row, column=5, value=str(result_detail['見積番号']) + str(result_detail['仕分番号']) + '-' + str(result_detail['見積行番号']))
