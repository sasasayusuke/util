...
def test_model_user():
    ...
    user_item_by_query_cognito = UserModel.cognito_id_index.query(user.cognito_id)
    user_item_by_query_email = UserModel.data_type_email_index.query(
        data_type, user.email
    )
    user_item_by_query_name = UserModel.data_type_name_index.query(data_type, user.name)
