...
from app.utils import date as util_date
...
class SchedulesService:
    ...
    def create_support_schedules(
        item: CreateSupportSchedulesRequest, project_id: str, current_user: AdminModel
    ) -> CreateSupportSchedulesResponse:
        """案件の支援スケジュールを指定期間もしくは指定日で作成します。

        Args:
            item (CreateSupportSchedulesRequest): 登録内容
            project_id (str): 案件ID
        ...
        if not SchedulesService.is_visible_project(current_user, project_id):
            logger.warning(
                f"CreateSupportSchedule forbidden. current_user:{current_user.id} project_id: {project_id}"
            )
            raise HTTPException(
        ...
        try:
            ...
            logger.warning(
                f"CreateSupportSchedule project not found. project_id: {project_id}"
            )
        ...
        return CreateSupportSchedulesResponse(message="OK")
        ...
        for karte in karte_list:
            ...
            schedule_list.append(
                ProjectSupportScheduleInfo(
                    ...
                    is_accessible_karte_detail=KartenService.is_visible_karte(
    ...
    @staticmethod
    def update_support_schedules_by_id_and_date(
        ...
        if not SchedulesService.is_visible_project(current_user, karte.project_id):
            logger.warning(
                f"UpdateSupportSchedule forbidden. current_user:{current_user.id} karte_id: {karte_id} project_id: {karte.project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
        ...
        if version != karte.version:
            logger.warning(
                f"UpdateSupportSchedule conflict. request_ver:{version} project_ver: {karte.version}"
            )
    ...
    def delete_support_schedules_by_id_date(
        ...
        if not SchedulesService.is_visible_project(current_user, karte.project_id):
            logger.warning(
                f"DeleteSupportSchedule forbidden. current_user:{current_user.id} karte_id: {karte_id} project_id: {karte.project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
        ...
        if query_params.version != karte.version:
            logger.warning(
                f"DeleteSupportSchedule conflict. request_ver:{query_params.version} karte_ver: {karte.version}"
            )
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Conflict")
    ...
    def create_survey_schedules(
        ...
        if not SchedulesService.is_visible_project(current_user, project_id):
            logger.warning(
                f"CreateSurveySchedule forbidden. current_user:{current_user.id} project_id: {project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
    ...
    def get_survey_schedules_during_period(
        ...
        """
        now = util_date.get_datetime_now()
        # 開催日時の判定（既に支援開始されてる場合は、現在日時から計算）
        if now.strftime("%Y/%m/%d") > support_start_date:
            start_date = now
        else:
            ...
            if timing == ScheduleTiming.MONTHLY:
                if -30 <= int(request_date) <= -1:
                    ...
                    (
                        ...
                        survey_limit_date_list,
                    ) = util_date.get_continuous_business_date_for_each_month(
                        start_date=start_date,
                        end_date=datetime.strptime(support_end_date, "%Y/%m/%d"),
                    ...
                    # 毎月○日:1-31
                    survey_request_date_list = (
                        util_date.get_continuous_date_for_each_month(
                            start_date=start_date,
                            end_date=datetime.strptime(support_end_date, "%Y/%m/%d"),
                            offset_day=int(request_date),
                        )
                ...
                survey_request_date_list = util_date.get_continuous_date_for_each_week(
                    start_date=start_date,
                    end_date=datetime.strptime(support_end_date, "%Y/%m/%d"),
                ...
                try:
                    ...
                    try:
                        (
                            survey_request_date_list,
                            survey_limit_date_list,
                        ) = util_date.get_pair_business_date_for_once(
                            current_datetime=now,
                            ref_day=int(limit_date),
                            offset_day=int(request_date),
                        )
    ...
    def get_survey_schedules_by_id(
        ...
        for survey in survey_list:
            # 匿名アンケートの再送信フラグ
            anonymous_flag: bool = SchedulesService.get_anonymous_surveys(survey)
            ...
            schedule_list.append(
                ProjectSurveyScheduleInfo(
                    ...
                    is_resend_anonymous_survey=anonymous_flag,
        ...
        try:
            ...
            raise HTTPException(
                status_code=api_status.HTTP_404_NOT_FOUND, detail="project not found"
            )
    ...
    @staticmethod
    def get_anonymous_surveys(survey: ProjectSurveyModel) -> bool:
        """匿名アンケートの再送信が可能かどうか判定する

        Args:
            survey (ProjectSurveyModel): 案件アンケート

        Returns:
            bool True: 再送信可能, False: 不可能
        """
        # 送信済みでない場合、再送信不可
        ...
        # 匿名アンケートでない場合、再送信不可
        if not (not survey.answer_user_id and survey.dedicated_survey_user_email):
            return False
    ...
    def update_survey_schedules_by_id_and_date(
        ...
        """Put /schedules/survey/{project_id}/{year}/{month} 「アンケート」スケジュール更新API
        ...
        if not SchedulesService.is_visible_project(current_user, survey.project_id):
            logger.warning(
                f"UpdateSurveySchedule forbidden. current_user:{current_user.id} project_id: {survey.project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
        ...
        if query_params.version != survey.version:
            logger.warning(
                f"UpdateSurveySchedule conflict. request_ver:{query_params.version} project_ver: {survey.version}"
            )
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Conflict")
    ...
    def delete_survey_schedules_by_id_date(
        ...
        Args:
            survey_id (str): スケジュールID クエリパラメータで指定
            version (int): ロックキー（楽観ロック制御） クエリパラメータで指定(>=1)
            current_user (Behavior, optional): 認証済みのユーザー
        ...
        if not SchedulesService.is_visible_project(current_user, survey.project_id):
            logger.warning(
                f"DeleteSurveySchedule forbidden. current_user:{current_user.id} project_id: {survey.project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
        ...
        if query_params.version != survey.version:
            logger.warning(
                f"DeleteSurveySchedule conflict. request_ver: {query_params.version} project_ver: {survey.version}"
            )
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Conflict")
    ...
    @staticmethod
    def get_limit_date(send_date: str, survey_limit_date: int) -> str:
    ...
    def bulk_create_survey_schedules(
        ...
        if not SchedulesService.is_visible_project(current_user, project_id):
            logger.warning(
                f"BulkCreateSurveySchedule forbidden. current_user:{current_user.id} project_id: {project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
        ...
        try:
            ...
            logger.warning(
                f"BulkCreateSurveySchedule project not found. project_id: {project_id}"
            )
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    ...
    def bulk_update_survey_schedules(
        ...
        if not SchedulesService.is_visible_project(current_user, project_id):
            logger.warning(
                f"BulkUpdateSurveySchedule forbidden. current_user:{current_user.id} project_id: {project_id}"
            )
            raise HTTPException(
                status_code=api_status.HTTP_403_FORBIDDEN, detail="Forbidden"
            )
        ...
        try:
            ...
            logger.warning(
                f"BulkUpdateSurveySchedule project not found. project_id: {project_id}"
            )
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
        ...
        if (project.survey_group_id is None) or (not project.survey_group_id):
            logger.warning(
                "BulkUpdateSurveySchedule bad request. survey_group_id is None"
            )
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST)
    ...
    @staticmethod
    def check_and_delete_survey_group_id_of_many_project(
        project_model_list: List[ProjectModel], current_user_id: str
    ) -> None:
        """[複数project指定] 案件情報のsurvey_group_idに紐づく案件アンケートが存在しない場合、案件情報のsurvey_group_idを削除.

        Args:
            project_model_list (List[ProjectModel]): 案件モデルのリスト
            current_user_id (str): ユーザID
        ...
        """
        # 案件情報の取得
        project_id_list = [project.id for project in project_model_list]
        survey_list: List[ProjectSurveyModel] = []
        if project_id_list:
            # 指定された案件のアンケートを取得
            # IN句の上限が100のため、100ずつ分割して取得
            separate_unit: int = 100
            separated_project_id_list: List[List[str]] = [
                project_id_list[i : i + separate_unit]
                for i in range(0, len(project_id_list), separate_unit)
            ]
            for id_list in separated_project_id_list:
                filter_condition = None
                filter_condition &= ProjectSurveyModel.project_id.is_in(*id_list)
                survey_list.extend(
                    list(ProjectSurveyModel.scan(filter_condition=filter_condition))
                )
        ...
        with ProjectModel.batch_write() as project_batch:
            for project in project_model_list:
                if project.survey_group_id:
                    # 案件に紐づく案件アンケートの survey_group_id 存在チェック
                    survey_list_of_target: List[ProjectSurveyModel] = []
                    for survey in survey_list:
                        if (
                            survey.project_id == project.id
                            and survey.survey_group_id == project.survey_group_id
                        ):
                            survey_list_of_target.append(survey)

                    # 案件アンケートにsurvey_group_idが存在しない場合
                    if not survey_list_of_target:
                        # 案件の survey_group_id を削除
                        project.survey_group_id = None
                        project.update_at = update_datetime
                        project.update_id = current_user_id
                        project_batch.save(project)

    @staticmethod
    def update_multiple_survey_schedules(
        item: UpdateMultipleSurveySchedulesRequest,
        current_user: AdminModel,
    ) -> OKResponse:
        """Put /schedules/survey/multiple 「アンケート」スケジュール複数登録・更新API

        Args:
            item (UpdateMultipleSurveySchedulesRequest): 更新内容
            current_user (Behavior, optional): 認証済みのユーザー
        ...
        # 一括更新件数の上限チェック
        if len(item.add) + len(item.update) > 500:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="The maximum number of updates has exceeded 500",
            )

        # 更新対象のアンケート情報を取得
        survey_update_list: List[ProjectSurveyModel] = []
        survey_id_model_map: dict[str, ProjectSurveyModel] = {}
        if item.update:
            item_keys = [(info.survey_id, DataType.SURVEY) for info in item.update]
            survey_update_list = list(ProjectSurveyModel.batch_get(item_keys))
            for survey in survey_update_list:
                survey_id_model_map[survey.id] = survey

        # 新規・更新処理対象の案件IDのリスト
        project_id_list: List[str] = []
        for add_info in item.add:
            project_id_list.append(add_info.project_id)
        for survey in survey_update_list:
            project_id_list.append(survey.project_id)
        # 重複を排除
        project_id_list = list(set(project_id_list))

        # 案件情報の取得（対象案件のみ）
        project_list: List[ProjectModel] = []
        project_id_model_map: dict[str, ProjectModel] = {}
        if project_id_list:
            item_keys = [
                (project_id, DataType.PROJECT) for project_id in project_id_list
            ]
            project_list = list(ProjectModel.batch_get(item_keys))
        for project in project_list:
            project_id_model_map[project.id] = project

        # バリデーション
        # 案件IDの存在チェック
        for add_info in item.add:
            if add_info.project_id not in project_id_model_map.keys():
                logger.warning(
                    f"UpdateMultipleSurveySchedules not found. Project id not found. project_id:{add_info.project_id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"Project id not found. project_id:{add_info.project_id}",
                )

        # アンケートIDの存在チェック
        for update_info in item.update:
            if update_info.survey_id not in survey_id_model_map.keys():
                logger.warning(
                    f"UpdateMultipleSurveySchedules not found. Survey id not found. survey_id:{update_info.survey_id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"Survey id not found. survey_id:{update_info.survey_id}",
                )

        for survey in survey_update_list:
            # 更新可否チェック(送信済のアンケートは更新不可)
            if survey.actual_survey_request_datetime:
                logger.warning(
                    f"UpdateMultipleSurveySchedules bad request. Surveys that have been sent cannot be updated. survey_id:{survey.id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Surveys that have been sent cannot be updated. survey_id:{survey.id}",
                )

            # 更新対象として指定されたアンケートが、以下のアンケート種別以外の場合はエラー
            # ・サービスアンケート
            # ・修了アンケート
            if survey.survey_type not in [SurveyType.SERVICE, SurveyType.COMPLETION]:
                logger.warning(
                    f"UpdateMultipleSurveySchedules bad request. Only surveys whose survey type is service and completion can be specified. survey_id:{survey.id}, survey_type:{survey.survey_type}"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Only surveys whose survey type is service and completion can be specified. survey_id:{survey.id}, survey_type:{survey.survey_type}",
                )
        ...
        try:
            # yyyy/mm/dd形式
            send_date_datetime: datetime = datetime.strptime(item.send_date, "%Y/%m/%d")
            # 送信日
            update_send_date: str = item.send_date
            # 集計月
            update_year_month: str = send_date_datetime.strftime("%Y/%m")
            # 回答期限日
            update_limit_date: str = SchedulesService.get_limit_date(
                item.send_date, item.survey_limit_date
            )
            # 算出基準年月(YYYY/MM)
            calc_base_year_month: str = send_date_datetime.strftime("%Y/%m")
            ...
            try:
                ...
                # 送信日
                update_send_date: str = survey_request_date_list[0]
                # 集計月
                item_date_split = survey_request_date_list[0].split("/")
                update_year_month: str = (
                    item_date_split[0] + "/" + item_date_split[1].zfill(2)
                )
                # 回答期限日
                update_limit_date: str = survey_limit_date_list[0]
                # 算出基準年月(YYYY/MM)
                calc_base_year_month: str = now.strftime("%Y/%m")

            except ValueError:
                logger.warning(
                    "UpdateMultipleSurveySchedules bad request. You cannot set a send date that is earlier than the current date."
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="You cannot set a send date that is earlier than the current date.",
        ...
        # 送信日チェック(送信日≦現在日はエラー)
        calc_date = datetime.strptime(update_send_date, "%Y/%m/%d").date()
        if now.date() >= calc_date:
            # 算出された日時が現在日以前の場合
            logger.warning(
                f"UpdateMultipleSurveySchedules bad request. You cannot set a send date that is earlier than the current date. send_date(or calc_date):{update_send_date}"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"You cannot set a send date that is earlier than the current date. send_date(or calc_date):{update_send_date}",
            )

        # サービスアンケートのアンケートマスタ(最新)の取得
        latest_survey_master_service: SurveyMasterModel = (
            SurveyMasterModel.get_latest_survey_masters(survey_type=SurveyType.SERVICE)
        )
        # 修了アンケートのアンケートマスタ(最新)の取得
        latest_survey_master_completion: SurveyMasterModel = (
            SurveyMasterModel.get_latest_survey_masters(
        ...
        # ユーザ情報の取得
        user_id_name_map: dict = {}
        user_id_model_map: dict = {}
        user_list: List[UserModel] = list(UserModel.scan())
        for user in user_list:
            user_id_name_map[user.id] = user.name
            user_id_model_map[user.id] = user

        # 支援者組織
        supporter_organizations_id_name_list: List[dict] = (
            CachedDbItems.ReturnSupporterOrganizations()
        )
        supporter_organizations_id_name_map: dict = {}
        for supporter_organizations in supporter_organizations_id_name_list:
            supporter_organizations_id_name_map[supporter_organizations["id"]] = (
                supporter_organizations["name"]
            )
        # サービス種別
        service_types_id_name_list: List[dict] = CachedDbItems.ReturnServiceTypes()
        service_types_id_name_map: dict = {}
        for service_types in service_types_id_name_list:
            service_types_id_name_map[service_types["id"]] = service_types["name"]

        update_datetime = datetime.now()
        # 案件アンケートの登録・更新
        # 一括登録ではないため、スケジュールグループID（survey_group_id）は登録しない。更新の場合はクリアする。
        with ProjectSurveyModel.batch_write() as project_survey_batch:
            # 新規登録処理
            # 新規登録時は種別を自動判定
            for add_info in item.add:
                # 案件情報
                project = project_id_model_map.get(add_info.project_id)

                if project:
                    db_main_supporter_user: SupporterUserAttribute
                    if project.main_supporter_user_id is not None:
                        main_supporter_user: UserModel = user_id_model_map.get(
                            project.main_supporter_user_id
                        )
                        if main_supporter_user:
                            main_supporter_user_organization_id: str = ""
                            list_main_supporter_user_organization_name: List = []
                            main_supporter_user_organization_name: str = ""
                            ...
                            list_main_supporter_user_organization_name = [
                                supporter_organizations_id_name_map.get(org_id, "")
                                for org_id in main_supporter_user.supporter_organization_id
                            ]
                            main_supporter_user_organization_name = ";".join(
                                list_main_supporter_user_organization_name
                            )
                    ...
                    if project.supporter_user_ids is not None:
                        for user_ids_itr in project.supporter_user_ids:
                            # 支援者組織
                            supporter_user: UserModel = user_id_model_map.get(
                                user_ids_itr
                            )
                            supporter_user_organization_id: str = ""
                            list_supporter_user_organization_name: List = []
                            supporter_user_organization_name: str = ""

                            supporter_user_organization_id = ";".join(
                                supporter_user.supporter_organization_id
                            )

                            list_supporter_user_organization_name = [
                                supporter_organizations_id_name_map.get(org_id, "")
                                for org_id in supporter_user.supporter_organization_id
                            ]
                            supporter_user_organization_name = ";".join(
                                list_supporter_user_organization_name
                            )
                            ...
                            support_users_list.append(
                                SupporterUserAttribute(
                                    id=user_ids_itr,
                                    name=user_id_name_map.get(user_ids_itr, ""),
                                    organization_id=supporter_user_organization_id,
                                    organization_name=supporter_user_organization_name,
                                )
                            )
                    else:
                        support_users_list = None
                    ...
                    # 1ヵ月前を取得
                    support_end_before_month = support_date_to_datetime - relativedelta(
                        months=1
                    )
                    service_survey_end: datetime.date = get_last_date_of_month(
                        support_end_before_month.year, support_end_before_month.month
                    )
                    # 算出基準年月日
                    # 組み合わせにより送信日が翌月になる可能性があるため基準となる年月で判定
                    calc_base_date: datetime.date = datetime.strptime(
                        calc_base_year_month + "/01", "%Y/%m/%d"
                    ).date()
                    latest_survey_master: SurveyMasterModel = None
                    # 支援終了月(または、それ以降)の場合、修了アンケート。それ以外はサービスアンケート。
                    if service_survey_end < calc_base_date:
                        latest_survey_master = latest_survey_master_completion
                    else:
                        latest_survey_master = latest_survey_master_service
                    ...
                    project_survey_batch.save(
                        ProjectSurveyModel(
                            ...
                            survey_master_id=latest_survey_master.id,
                            survey_master_revision=latest_survey_master.revision,
                            survey_type=latest_survey_master.type,
                            project_id=project.id,
                            project_name=project.name,
                            customer_success=project.customer_success,
                            supporter_organization_id=project.supporter_organization_id,
                            supporter_organization_name=supporter_organizations_id_name_map.get(
                                project.supporter_organization_id, ""
                            ),
                            support_date_from=project.support_date_from,
                            support_date_to=project.support_date_to,
                            ...
                            sales_user_id=project.main_sales_user_id,
                            service_type_id=project.service_type,
                            service_type_name=service_types_id_name_map.get(
                                project.service_type, ""
                            ),
                            answer_user_id=project.main_customer_user_id,
                            answer_user_name=user_id_name_map.get(
                                project.main_customer_user_id, None
                            ),
                            customer_id=project.customer_id,
                            customer_name=project.customer_name,
                            points=PointsAttribute(
                            ...
                            summary_month=update_year_month,
                            plan_survey_request_datetime=SchedulesService.make_request_datetime(
                                update_send_date
                            ),
                            plan_survey_response_datetime=SchedulesService.make_request_datetime(
                            ...
                            this_month_type=project.this_month_type,
                            no_send_reason=project.no_send_reason,
                            dedicated_survey_user_name=project.dedicated_survey_user_name,
                            dedicated_survey_user_email=project.dedicated_survey_user_email,
                            create_id=current_user.id,
                            create_at=update_datetime,
                            update_at=update_datetime,
                            version=1,
            ...
            for survey in survey_update_list:
                survey.plan_survey_request_datetime = (
                    SchedulesService.make_request_datetime(update_send_date)
                )
                survey.plan_survey_response_datetime = (
                    SchedulesService.make_request_datetime(update_limit_date)
                )
                survey.summary_month = update_year_month
                survey.survey_group_id = None
                survey.update_at = update_datetime
                survey.update_id = current_user.id
                survey.version += 1

                project_survey_batch.save(survey)

        # 案件情報のsurvey_group_idに紐づく案件アンケートが存在しない場合、案件情報のsurvey_group_idを削除
        SchedulesService.check_and_delete_survey_group_id_of_many_project(
            project_model_list=project_list, current_user_id=current_user.id
        )
    ...
    def delete_multiple_survey_schedules(
        item: DeleteMultipleSurveySchedulesRequest,
        current_user: AdminModel,
    ) -> OKResponse:
        """Put /schedules/survey/multiple 「アンケート」スケジュール複数削除API

        Args:
            item (DeleteMultipleSurveySchedulesRequest): 削除情報
            current_user (Behavior, optional): 認証済みのユーザー
        ...
        # 一括更新件数の上限チェック
        if len(item.survey_ids) > 500:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="The maximum number of deletes has exceeded 500",
            )

        # 削除対象のアンケート情報を取得
        survey_delete_list: List[ProjectSurveyModel] = []
        if item.survey_ids:
            item_keys = [(survey_id, DataType.SURVEY) for survey_id in item.survey_ids]
            survey_delete_list = list(ProjectSurveyModel.batch_get(item_keys))

        # バリデーション
        for survey in survey_delete_list:
            # 削除可否チェック(送信済のアンケートは削除不可)
            if survey.actual_survey_request_datetime:
                logger.warning(
                    f"DeleteMultipleSurveySchedules bad request. Surveys that have been sent cannot be deleteed. survey_id:{survey.id}"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Surveys that have been sent cannot be deleted. survey_id:{survey.id}",
                )
        ...
        if survey_delete_list:
            item_keys = [
                (survey.project_id, DataType.PROJECT) for survey in survey_delete_list
            ]
            project_list = list(ProjectModel.batch_get(item_keys))
