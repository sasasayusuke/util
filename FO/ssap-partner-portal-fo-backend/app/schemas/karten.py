...
class GetKartenLatestResponse(CustomBaseModel):
    """最近更新されたカルテの一覧取得レスポンスクラス"""

    karte_id: str = Field(
        ..., title="案件カルテID", example="89cbe2ed-f44c-4a1c-9408-c67b0ca2270d"
    )
    project_id: str = Field(
        ..., title="案件ID", example="99cbe2ed-f44c-4a1c-9408-c67b0ca2270d"
    )
    project_name: str = Field(..., title="案件名", example="プロジェクトA")
    customer_name: str = Field(..., title="取引先名", example="取引先名")
    date: str = Field(..., title="支援日", example="2022/05/20")
    start_time: str = Field(..., title="開始時間", example="10:00")
    end_time: str = Field(..., title="終了時間", example="13:00")
    day: str = Field(..., title="曜日", example="火")
    last_update_datetime: datetime = Field(
        None, title="最終更新日時", example="2020-10-23T03:21:39.356872+09:00"
    )
    draft_supporter_name: str = Field(..., title="最終更新者名", example="最終更新者名")
...
class GetKartenByIdResponse(CustomBaseModel):
    ...
    memo: str = Field(None, title="マネジメントへのリクエスト")
    ...
    task: str = Field(None, title="現状の課題・所感・申し送り")
    ...
    update_at: datetime = Field(
        None, title="更新日時（JST）", example="2020-10-23T03:21:39.356872+09:00"
    )
    version: int = Field(
...
class UpdateKarteByIdRequest(CustomBaseModel):
    """IDをもとに更新する案件カルテリクエストクラス"""
    ...
    man_hour: float = Field(None, title="実施時間（工数h）")
    customers: str = Field(None, title="お客様（敬称略）")
    support_team: str = Field(None, title="SAP支援チーム（敬称略）")
    detail: str = Field(None, title="支援実施内容")
    feedback: str = Field(None, title="フィードバック")
    homework: str = Field(None, title="宿題")
    documents: List[DocumentInfo] = Field(None, title="資料")
    deliverables: List[DeliverablesInfo] = Field(None, title="成果物")
    memo: str = Field(None, title="マネジメントへのリクエスト")
    human_resource_needed_for_customer: str = Field(
        None, title="お客様に不足している人的リソース"
    )
    ...
    location: Location = Field(None, title="場所")
    is_draft: bool = Field(..., title="下書きか否か")
    is_notify_update_karte: bool = Field(..., title="カルテの更新を通知するか否か")


class UpdateKarteByIdQuery(CustomBaseModel):
    """案件カルテ更新クエリパラメータクラス"""

    version: int = Query(..., title="ロックキー（楽観ロック制御）", example=1, ge=1)


class UpdateKarteByIdResponse(CustomBaseModel):
    """案件カルテ更新レスポンスクラス"""
